...
import { draftRecipe, loadEngine } from './engine.js?v=148';
...
function downloadFactoryPack() {
  ...
  a.href = `factory/${pack.file}?v=148`;
...
async function loadRecipe(entry) {
  const res = await fetch(`recipes/${entry.file}?v=148`);
...
async function init() {
  ...
  try {
    const res = await fetch('recipes/index.json?v=148');
  ...
  try {
    const fres = await fetch('factory/index.json?v=148');
