...
import { draftRecipe, loadEngine } from './engine.js?v=152';
...
function downloadFactoryPack() {
  ...
  a.href = `factory/${pack.file}?v=152`;
...
async function loadRecipe(entry) {
  const res = await fetch(`recipes/${entry.file}?v=152`);
...
async function init() {
  ...
  try {
    const res = await fetch('recipes/index.json?v=152');
  ...
  try {
    const fres = await fetch('factory/index.json?v=152');
