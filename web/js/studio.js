...
import { draftRecipe, loadEngine } from './engine.js?v=143';
...
function downloadFactoryPack() {
  ...
  a.href = `factory/${pack.file}?v=143`;
...
async function loadRecipe(entry) {
  const res = await fetch(`recipes/${entry.file}?v=143`);
...
async function init() {
  ...
  try {
    const res = await fetch('recipes/index.json?v=143');
  ...
  try {
    const fres = await fetch('factory/index.json?v=143');
