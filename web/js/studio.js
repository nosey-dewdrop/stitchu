...
import { draftRecipe, loadEngine } from './engine.js?v=138';
...
function downloadFactoryPack() {
  ...
  a.href = `factory/${pack.file}?v=138`;
...
async function loadRecipe(entry) {
  const res = await fetch(`recipes/${entry.file}?v=138`);
...
async function init() {
  ...
  try {
    const res = await fetch('recipes/index.json?v=138');
  ...
  try {
    const fres = await fetch('factory/index.json?v=138');
