// src/kalip/kalip.mjs — SILUET -> KALIP (0-K.10.e). Flat genislikleri kaliba HEDEF, pens ve bolluk oradan.
//   node src/kalip/kalip.mjs <set> [no...]
...
import { noktaBeden } from '../ciz/ciz-cli.mjs';
...
const BODY = JSON.parse(readFileSync('kanun/1309-beden.json', 'utf8'));
...
for (const no of nolar) {
  ...
  const taban = eski.tabanGraf || 'kanun/1309-taban-graf.json';
  ...
  for (let deneme = 0; deneme < 3; deneme++) {
    ...
    r = spawnSync('motor/build/grafuygula', [taban, `${d}/ops.json`, '--id', `foto-${sha.slice(0, 8)}`, '--hedef', `${d}/hedefler.json`, '--beden', 'gercek36'], { encoding: 'utf8', maxBuffer: 64e6 });
    if (r.status === 0) break;
  ...
  const c = spawnSync('motor/build/grafciz', [taban, '--ops', `${d}/ops.json`, '--hedef', `${d}/hedefler.json`, 'gercek36', 'kalip'], { encoding: 'utf8', maxBuffer: 64e6 });
  ...
  yeni.push(`cizim flat = src/ciz/ciz-cli.mjs (okuma KOSU/onbellek/siluet-${sha.slice(0, 12)}….json; motor flat'e girmez)`);
