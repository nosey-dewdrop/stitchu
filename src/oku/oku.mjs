#!/usr/bin/env node
// src/oku/oku.mjs — FOTOGRAF/PROMPT -> SILUET OKUMASI. (11 Eyl 2026, G1)
//
...
//
// KULLANIM
//   node src/oku/oku.mjs <fotograf.png>            fotografi okut
//   node src/oku/oku.mjs --prompt "<metin>"        metin tarifinden oku
//   node src/oku/oku.mjs --girdi <okuma.json>      okuyucu cagrisi YOK:
...
//
// CIKTI: stdout'a kanun/1309-siluet.json semasina uyan JSON.
...
// ---------------------------------------------------------------- sema dogrulama
const SILUET = JSON.parse(readFileSync(`${KOK}/kanun/1309-siluet.json`, 'utf8'));
const X_RE = new RegExp(SILUET.koordinat.xIfade);
const Y_RE = new RegExp(SILUET.koordinat.yIfade);
const LANDMARKLAR = new Set(
  Object.keys(JSON.parse(readFileSync(`${KOK}/kanun/1309-beden.json`, 'utf8')).bedenler.croquis36.landmarklar)
...
async function ana() {
  ...
  if (!fotoYolu && !metin && !girdiJson) {
    ol('ERR_GIRDI', 'kullanim: node src/oku/oku.mjs <foto> | --prompt "<metin>" | --girdi <json>');
  }
