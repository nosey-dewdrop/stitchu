...
// aboneligidir (src/oku/oku.mjs -> 'claude -p' alt sureci).
...
const SET = 'yerel';                          // cikti/yerel/<no>/
const SET_KOK = `${KOK}/cikti/${SET}`;
...
const TABAN_TOPOLOJI = `${KOK}/kanun/1309-taban-topoloji.json`;
...
async function isiKos(id, adi, bayt) {
  ...
  const t0 = Date.now();
  const o = await komut(['src/oku/oku.mjs', relative(KOK, fotoYolu)]);
  ...
  const f = await komut(['src/ciz/uret.mjs', SET, String(no)]);
  let kirmizi = [];
  ...
  const k = await komut(['src/kalip/kalip.mjs', SET, String(no)]);
  let ozet = null;
