...
//
// KALAN TEK KALEM BUDUR ve o kalem src/ciz/ciz-cli.mjs'in ta kendisidir —
// KOPYA DEGIL, TASINMIS hali. CLI (src/ciz/ciz-cli.mjs) artik bu dosyayi import
// eder; iki yerde iki geometri yok, tek geometri var.
//
// NE CIZER. Flat, GORDUGUNUN mankene (croquis36) cizilmis halidir. Okuma
// (kanun/1309-siluet.json semasi) noktalari manken landmark'larina ORANLA verir
...
//             kanunuKur(await (await fetch('/kanun/1309-beden.json')).json(),
//                       await (await fetch('/kanun/1309-siluet.json')).json());
//   node:     src/ciz/ciz-cli.mjs iki dosyayi diskten okuyup kanunuKur cagirir.

let BODY = null;   // kanun/1309-beden.json
let CIZ = null;    // kanun/1309-siluet.json .cizgi
let LM = null;     // croquis36 landmarklari
let KOLEVI = null; // kanun/1309-siluet.json .gorunum.kolevi  (kol evi = KENAR)
let ETEKUCU = null;// kanun/1309-siluet.json .gorunum.etekUcu (etek ucu kavisi)
...
export function kanunuKur(bodyV1, siluetV1) {
  if (!bodyV1 || !bodyV1.bedenler || !bodyV1.bedenler.croquis36) {
    throw new Error('ERR_KANUN_YOK: kanun/1309-beden.json verilmedi (bedenler.croquis36 yok)');
  }
  if (!siluetV1 || !siluetV1.cizgi) {
    throw new Error('ERR_KANUN_YOK: kanun/1309-siluet.json verilmedi (.cizgi yok)');
  }
  BODY = bodyV1;
  CIZ = siluetV1.cizgi;
  // KOL EVI ve ETEK UCU: sayilar burada DEGIL, kanun/1309-siluet.json'da yasar (bkz. AYNA SABITI YOK).
...
// ayni ifade baska bedende (kalip: gercek36) — src/kalip/kalip.mjs kullanir
...
// ---- nokta ifadeleri (kanun/1309-siluet.json koordinat)
