// src/ciz/ciz-cli.mjs — SILUET -> FLAT, NODE/CLI SARMALAYICISI.
//
...
//   1. kanun/1309-beden.json + kanun/1309-siluet.json'u DISKTEN okur ve
...
//
//   node src/ciz/ciz-cli.mjs <siluet.json> <cikis.svg>
//   import { ciz } from '../ciz/ciz-cli.mjs'; ciz(okuma) -> { svg, imza, kirmizi[] }
import { readFileSync, writeFileSync } from 'node:fs';
import { kanunuKur, kanunKurulduMu } from './ciz.js';
...
if (!kanunKurulduMu()) {
  kanunuKur(
    JSON.parse(readFileSync('kanun/1309-beden.json', 'utf8')),
    JSON.parse(readFileSync('kanun/1309-siluet.json', 'utf8')),
  );
}

export { ciz, nokta, noktaBeden, kanunuKur, kanunKurulduMu } from './ciz.js';
import { ciz } from './ciz.js';

if (process.argv[1] && process.argv[1].endsWith('siluet-ciz.mjs')) {
  const [girdi, cikis] = process.argv.slice(2);
  if (!girdi || !cikis) { console.error('kullanim: node src/ciz/ciz-cli.mjs <siluet.json> <cikis.svg>'); process.exit(2); }
