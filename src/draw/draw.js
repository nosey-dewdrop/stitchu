...
//
// KALAN TEK KALEM BUDUR ve o kalem src/draw/draw-cli.mjs'in ta kendisidir —
// KOPYA DEGIL, TASINMIS hali. CLI (src/draw/draw-cli.mjs) artik bu dosyayi import
// eder; iki yerde iki geometri yok, tek geometri var.
//
// NE CIZER. Flat, GORDUGUNUN mankene (croquis36) cizilmis halidir. Okuma
// (law/1309-silhouette.json semasi) noktalari manken landmark'larina ORANLA verir
...
//             kanunuKur(await (await fetch('/law/1309-body.json')).json(),
//                       await (await fetch('/law/1309-silhouette.json')).json());
//   node:     src/draw/draw-cli.mjs iki dosyayi diskten okuyup kanunuKur cagirir.

let BODY = null;   // law/1309-body.json
let CIZ = null;    // law/1309-silhouette.json .cizgi
let LM = null;     // croquis36 landmarklari
let KOLEVI = null; // law/1309-silhouette.json .gorunum.kolevi  (kol evi = KENAR)
let ETEKUCU = null;// law/1309-silhouette.json .gorunum.etekUcu (etek ucu kavisi)
...
export function kanunuKur(bodyV1, siluetV1) {
  if (!bodyV1 || !bodyV1.bedenler || !bodyV1.bedenler.croquis36) {
    throw new Error('ERR_KANUN_YOK: law/1309-body.json verilmedi (bedenler.croquis36 yok)');
  }
  if (!siluetV1 || !siluetV1.cizgi) {
    throw new Error('ERR_KANUN_YOK: law/1309-silhouette.json verilmedi (.cizgi yok)');
  }
  BODY = bodyV1;
  CIZ = siluetV1.cizgi;
  // KOL EVI ve ETEK UCU: sayilar burada DEGIL, law/1309-silhouette.json'da yasar (bkz. AYNA SABITI YOK).
...
// ayni ifade baska bedende (kalip: gercek36) — src/pattern/pattern.mjs kullanir
...
// ---- nokta ifadeleri (law/1309-silhouette.json koordinat)
