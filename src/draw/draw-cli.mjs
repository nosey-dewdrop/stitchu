// src/draw/draw-cli.mjs — SILUET -> FLAT, NODE/CLI SARMALAYICISI.
//
...
//   1. law/1309-body.json + law/1309-silhouette.json'u DISKTEN okur ve
...
//
//   node src/draw/draw-cli.mjs <siluet.json> <cikis.svg>
//   import { ciz } from '../draw/draw-cli.mjs'; ciz(okuma) -> { svg, imza, kirmizi[] }
import { readFileSync, writeFileSync } from 'node:fs';
import { kanunuKur, kanunKurulduMu } from './draw.js';
...
if (!kanunKurulduMu()) {
  kanunuKur(
    JSON.parse(readFileSync('law/1309-body.json', 'utf8')),
    JSON.parse(readFileSync('law/1309-silhouette.json', 'utf8')),
  );
}

export { ciz, nokta, noktaBeden, kanunuKur, kanunKurulduMu } from './draw.js';
import { ciz } from './draw.js';

if (process.argv[1] && process.argv[1].endsWith('siluet-ciz.mjs')) {
  const [girdi, cikis] = process.argv.slice(2);
  if (!girdi || !cikis) { console.error('kullanim: node src/draw/draw-cli.mjs <siluet.json> <cikis.svg>'); process.exit(2); }
