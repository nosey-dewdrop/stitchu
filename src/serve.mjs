...
// aboneligidir (src/read/read.mjs -> 'claude -p' alt sureci).
...
const SET = 'yerel';                          // output/yerel/<no>/
...
async function isiKos(id, adi, bayt) {
  ...
  const t0 = Date.now();
  const o = await komut(['src/read/read.mjs', relative(KOK, fotoYolu)]);
  ...
  const f = await komut(['src/draw/run.mjs', SET, String(no)]);
  let kirmizi = [];
  ...
  const k = await komut(['src/pattern/pattern.mjs', SET, String(no)]);
  let ozet = null;
