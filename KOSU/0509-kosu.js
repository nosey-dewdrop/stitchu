...
async function el(gorev, adim, ek) {
 ...
 1. ZORUNLU ILK KOMUT (atlanamaz, baska hicbir sey bundan once yapilmaz): git tag -f ${_tagAd}-once HEAD && git push -f origin ${_tagAd}-once. Tag adi TAM OLARAK "${_tagAd}-once" — kisaltma, adim numarasina indirgeme, "zaten var" diye atlama YOK; bu tag butcenin baslangic noktasidir, yanlissa butce yanlis sayilir ve adim haksiz yere duser. Attiktan sonra "git tag -l ${_tagAd}-once" ciktisini yapilan[] icine yaz. (yeniden acilis/tekrar ise tag ILERI tasinir: butce bu acilistan sayilir); git push -f origin --tags. REFERANS KILIDI (KARAR Q2, 2026-09-06 A2 karar ajani — kilit COMMIT'TE TASINMAZ, chmod git nesnesi degildir; bu yuzden kilit her alt adim basinda FIILEN yeniden kurulur ve durumu commit mesajindan DEGIL H7 ciktisindan okunur):
   (a) FIILEN KOS: bash ${KAPI} --kilit ${JSON.stringify(ek && ek.izin || '')} (yoksa: chmod -R a-w contract engine/tests engine/golden-reference.csv engine/src/grafdogrula.* engine/src/solver_utils.* 2>/dev/null; izin listesindekilere chmod u+w). Ciktinin "kilit: N dosya salt-okunur, izin listesinden M dosya yazilabilir" satirini AYNEN yapilan[] icine yaz (parafraz degil, satirin kendisi).
...
async function adimKos(faz, ekNot) {
  ...
  if (ALT[faz.no]) {   // 9.7: buyuk adim, alt adim basina isci; hakem her alt adimi olcer
    ...
    for (const alt of ALT[faz.no]) {
      ...
      const bAlt = await el('basla', faz, { izin: (faz.izinAlt && faz.izinAlt[alt]) || faz.izin || '', altAdim: alt })
