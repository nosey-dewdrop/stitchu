...
export function uret(sha, cikisDizin) {
  ...
  const r = spawnSync('engine/build/grafuygula', [TABAN, opsYol, '--id', `foto-${sha.slice(0, 8)}`], { encoding: 'utf8', maxBuffer: 64e6 });
  ...
  if (r.status !== 0) {
    ...
    return { okuma, opsYol, grafYol: null, ops, motor, hedefler: okuma.hedefler || [] };
  }
  ...
  return { okuma, opsYol, grafYol, ops, motor, hedefler: okuma.hedefler || [] };
}

export function ciz(opsYol, cikisDizin) {
  const c = {};
  for (const [ad, body, mod] of [['flat', 'croquis36', 'flat'], ['kalip-36', 'gercek36', 'kalip']]) {
    try {
      const svg = execFileSync('engine/build/grafciz', [TABAN, '--ops', opsYol, body, mod], { encoding: 'utf8', maxBuffer: 64e6, stdio: ['ignore', 'pipe', 'pipe'] });
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const c = r.grafYol ? ciz(r.opsYol, dizin) : { flat: { durum: 'KOSMADI' }, 'kalip-36': { durum: 'KOSMADI' } };
