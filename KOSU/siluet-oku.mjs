#!/usr/bin/env node
// KOSU/siluet-oku.mjs — FOTOGRAF/PROMPT -> SILUET OKUMASI. (11 Eyl 2026, G1)
//
...
//
// KULLANIM
//   node KOSU/siluet-oku.mjs <fotograf.png>            fotografi okut
//   node KOSU/siluet-oku.mjs --prompt "<metin>"        metin tarifinden oku
//   node KOSU/siluet-oku.mjs --girdi <okuma.json>      okuyucu cagrisi YOK:
...
//
// CIKTI: stdout'a contract/siluet-v1.json semasina uyan JSON.
...
const LANDMARKLAR = new Set(
  Object.keys(JSON.parse(readFileSync(`${KOK}/contract/body-v1.json`, 'utf8')).bedenler.croquis36.landmarklar)
...
async function ana() {
  ...
  if (!fotoYolu && !metin && !girdiJson) {
    ol('ERR_GIRDI', 'kullanim: node KOSU/siluet-oku.mjs <foto> | --prompt "<metin>" | --girdi <json>');
  }
