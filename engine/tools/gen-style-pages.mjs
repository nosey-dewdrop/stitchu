...
// Asset cache-bust version. This was the literal 84 until TUR 13, on a site
// standing at 136: running this generator exited 0 and rewound every styles
// page 52 bumps, so the browser was told to reuse css/js it should have
// refetched. Now read from web/ itself, which is the only record of the
// current version (scripts/deploy.sh bumps it there).
const V = siteVersion();
...
// sitemap.xml and robots.txt are NOT written here any more. This generator
// built them from a hardcoded page list plus web/patterns/svg/meta.json; when
// af49514 deleted that tree the try/catch swallowed it, and the sitemap left
// on disk kept advertising 22 URLs that 404 while omitting the 24 pages this
// very file produces. A sitemap must be derived from the site, not from a
// list a page generator happens to carry: engine/tools/gen-sitemap.mjs.

console.log(`generated ${STYLES.length} style pages + hub at ?v=${V} (run gen-sitemap.mjs to refresh sitemap.xml)`);
