...
// Collection detail pages and patch-note pages are generated by separate tools;
// scan their dirs so a regen here never drops their URLs (the class of bug that
// once shipped a sitemap missing 61 patches + every collection look).
let collectionPages = [];
try {
  collectionPages = readdirSync(join(ROOT, 'web', 'collections'))
    .filter((f) => f.endsWith('.html') && f !== 'index.html')
    .map((f) => `collections/${f}`);
} catch { /* collections not built yet */ }
let patchPages = [];
try {
  patchPages = readdirSync(join(ROOT, 'web', 'patches'))
    .filter((f) => f.endsWith('.html'))
    .map((f) => `patches/${f}`);
} catch { /* patches not built yet */ }

// Priority tiers: content pages that earn search traffic rank highest; index
// hubs mid; patch notes are numerous but lower intent. changefreq tells Google
// how often to recrawl. Every URL carries today's lastmod so an edit signals a
// recrawl.
const P_TOP = ['create.html', 'benchmark.html'];
function entry(p) {
  const bare = p.replace(/index\.html$/, '') || '';
  const loc = `${BASE}/${bare}`;
  let priority = '0.6', freq = 'weekly';
  if (bare === '') { priority = '0.9'; freq = 'weekly'; }
  else if (P_TOP.includes(bare)) { priority = '0.8'; freq = 'weekly'; }
  else if (/^(patterns|collections|styles)\/[a-z0-9-]+\.html$/.test(bare)) { priority = '0.7'; freq = 'weekly'; }
  else if (/^patches\//.test(bare)) { priority = '0.5'; freq = 'monthly'; }
  else if (/\/$/.test(bare) || bare.endsWith('.html')) { priority = '0.6'; freq = 'weekly'; }
...
// De-dupe (a listing hub can appear twice via different builders).
const seen = new Set();
const uniquePages = pages.filter((p) => { const k = p.replace(/index\.html$/, ''); if (seen.has(k)) return false; seen.add(k); return true; });
const sitemap = `<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
${uniquePages.map(entry).join('\n')}
</urlset>
`;
