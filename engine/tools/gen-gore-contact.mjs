...
const html = `<!doctype html><html lang=tr><head><meta charset=utf-8><meta name=robots content=noindex>` +
  ...
  `<p class=sub>Mihenk 5'lisinin 3. hedefi. Gode motorda ZATEN canlı (6-panel; taban golden'ı değişmiyor, bekçi engine/golden-diff.py). Bu halka flat KALEMİN gore'u çizmesini ekliyor: skirtStyle 'gore' → bel→hem eşit aralıklı panel seam'leri, her biri godet gibi dışa flare. Motor 6 panel kesiyor → flat 6 panel gösteriyor (listing = kalıp tutarlı). Tarif verme, SEÇ. Vekil taslağı: g1 (motor-tutarlı 6-gore midi).</p>` +
  `<div class=grid>${cells}</div>` +
  `<p class=note>Geometri doğrulandı: 6-gore CF + ±20/±40/±60 bel → ±31.6/±63.2/±94.8 hem, monoton flare (çapraz yok = üretilebilir). Panel seam DÜZ Q-flare; "seam eğrisi sert" dersen F3 eğri cilası hakkı var. Kapılar: ctest + engine/golden-diff.py; sayıları o aletler basar, bu sayfa için yeniden koşulmadı.</p>` +
  `</body></html>`;
