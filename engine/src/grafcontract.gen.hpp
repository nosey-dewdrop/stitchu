...
inline const char* kGrafContractJSON() { return R"stitchu({
  ...
  "_yasa": [
    ...
    "11. (2026-09-09, Damla karari) Motor taban grafi degil, taban + PRIMITIF EMIR LISTESINI uygular ve cizer (engine/build/grafuygula; grafciz --ops). Primitif kumesi geometri emirleridir ve giysi adi icermez; fotograf/cumle basina op eklenmez — bir girdi ifade edilemiyorsa kumede bir primitif eksiktir, sozluk acilmaz. Op'suz cizim kirmizidir (ERR_NO_OPS)."
  ],
  ...
  "tipler": {
    ...
    "Panel": {
      ...
      "alanlar": {
        ...
        "onto": {"tip": "string", "aciklama": {"tr": "Dolu ise panel bu KONAK panelin yuzune dikilir (aplike/ust-dikis: yama, band). Kenarlari Seam'e girmez; komsuluk ve 2B poz konaktan gelir. op addPanel.onto yazar.", "en": "If set, this panel is applied onto the named host panel's face; connectivity and placement come from the host."}},
        "darts": {"tip": "IcPens[]", "aciklama": {"tr": "IC HALKA PENSLER (2026-09-09): panelin dis halkasina degmeyen pens (balik pensi). op merge, kosunun icinde kalan dartLeg ciftlerini buraya tasir. Kalipta ic cizgi (katman 8); supresyon kapisinda agiz sayilir.", "en": "Interior (fish) darts not touching the outline; produced by op merge."}}
      }
    },
    "Edge": {
    ...
    "IcPens": {"aciklama": {"tr": "Ic halka pens: agiz from-to, yukari apeks apexUp, asagi apeks apexDown. apexDown agiz ortasina esitse ucgen (yalniz yukari).", "en": "Interior dart: mouth from-to, apexUp, apexDown."}, "alanlar": {"id": {"tip": "string", "zorunlu": true}, "from": {"tip": "RefPoint", "zorunlu": true}, "to": {"tip": "RefPoint", "zorunlu": true}, "apexUp": {"tip": "RefPoint", "zorunlu": true}, "apexDown": {"tip": "RefPoint", "zorunlu": true}}},
    "Seam": {
  ...
  "cozucu": {
    ...
    "hedef": {"_tanim": "vision-graf-v1 hedefler[] (halka orani, siluet olcumu) grafa oran olarak YAZILMAZ; cozucu (grafop.hpp hedefUygula) hedef orani halka BOLLUGUNA (Panel.ease mm) cevirir: ease(ring) = r x (obur halka + bollugu) - beden halkasi. Sonuc arama sinirina kirpilir, sapma raporlanir (grafdogrula --hedef 'hedef' satiri, sapmaTablosu). Kirpilan hedef sessizce yutulmaz: uygulanan ve istenen ayri yazilir.",
      "easeMinMM": {"deger": 0, "kaynak": "Dokuma giysi bedenden kucuk olamaz: bolluk alt siniri 0 mm (strec kumas A6 fabric-catalog ile ayri). Arama siniri, fizik sabiti degil."},
      "easeMaxKat": {"deger": 2.0, "kaynak": "Ust sinir = taban grafin o halkadaki bollugunun 2 kati (25 mm bel -> 50). DOGRULANMADI: fabric-catalog-v1 halka bollugu bandi A6'da baglaninca bu kat onunla degisir; sinir gevsetme yonunde secilmedi (siluet oranlari genelde DARALTMA ister)."}},
    "pens": {
      "_tanim": "cozPens (grafop.cpp) ve supresyon kapisi (grafdogrula.cpp), A4 2026-09-09. Pens agzi = panelin HALKA CIFTINDEKI supresyon x pensPayi; halka cifti panel uyeliginden okunur (panelin uye oldugu, landmark'li, bel disi govde halkasi — bele en yakini: etek icin kalca - bel, beden icin gogus - bel). ON/ARKA AYRI: supresyon bedenin arkaPay'i ile bolunur (arka = ust x arkaPay(ust) - alt x arkaPay(alt), on = kalan); on ve arka pens agzi bu yuzden farklidir, uydurulan on/arka sayisi yok. Yarim panel: kendi tarafinin supresyonunun yarisi, o taraftaki pens sayisina bolunur. KAPI (supresyon): tautolojik degil — cozucunun yazdigi agiz orani degil, dikilen alt halkanin (bel) cevresi ile bedenin cevresi + bollugu arasindaki EMILMEYEN fark olculur; |emilmeyen| <= toleranslar.dikisUzunlukMM.",
      "pensPayi": {
        "deger": 0.3333333333333333,
        "kaynak": "Aldrich temel blok: bel supresyonu pens ve yan dikis arasinda paylasilir, pens payi ~1/3. DOGRULANMADI: sayfa/olcumle teyit edilmedi; grafop.cpp'deki kPensPayi sabiti (1/3) contract'a tasindi, deger degismedi."
      }
    },
    "gevsetme": {
      ...
      "icProjeksiyon": {"deger": 4, "kaynak": "Bir iterasyon icinde SERT projeksiyon turunun (Gauss-Seidel) kac kez tekrarlandigi. Sayisal ic dongu, ESIK DEGIL: kisit hedefleri ve toleranslar bu bloktaki obur alanlardan gelir, bu yalniz turun kac kez suruldugudur. DOGRULANMADI (yayinlanmis kaynagi yok): 4 tur, birbirine bagli kisit zincirinde (bir dugum iki kisitte) artigi bir mertebe dusurur; 1 tur yakinsamayi maxIter'e yayar. Deger 4'ten DEGISMEZ; A2c'de solver_utils.cpp'deki constexpr int kIcProjeksiyon = 4 sabiti buraya tasindi (KARAR 3), kodda olcu sabiti kalmadi."},
  ...
  "oplar": {
    ...
    "reshapeEdge": {"args": {"panel": "string", "edge": "string", "from": "RefPoint?", "to": "RefPoint?", "control": "RefPoint[]?", "kind": "enum:edgeKind? (cut|seam|fold; dartLeg yazilamaz)", "finish": "enum:finish? (yalniz cut)"}, "tr": "Kenarin uc/kontrol noktalarini ve TURUNU yeniden yazar (yaka, etek ucu; kat kenarinin bir bolumu serbest kenar olur = yarik). cut'a cevrilen kenar finish ister; kat kenari kalmayan panel onFold=false, cutCount>=2.", "en": "Rewrite an edge's endpoints / controls / kind."},
    ...
    "sew": {"args": {"seam": "string", "a": "EdgeRef[]", "b": "EdgeRef[]", "reverse": "boolean (zorunlu)", "ratio": "number (varsayilan 1)", "easeMM": "number (varsayilan 0)"}, "tr": "DIK (2026-09-09): mevcut kenarlari yeni bir dikiste birlestirir. fold -> seam (kat acilir, parca 2 kesilir), cut -> seam (finish silinir). Ayni kenar iki tarafta olabilir (kendi ayna kopyasiyla dikis = on/arka orta kapanma); dogrulayici bunu yalniz closure ilan edilince kabul eder (ayna kapanmasi).", "en": "Sew existing edges into a new seam; fold/cut edges become seam edges."},
    "addPanel": {"args": {"panel": "Panel", "onto": "string? (konak panel: yuze dikili/aplike)"}, "tr": "PANEL EKLE (2026-09-09): kapali yeni panel. onto verilirse panel konak panelin YUZUNE dikilir (yama, band): kenarlari dikise girmez, komsuluk konak uzerinden kurulur, cizimde konagin pozunu alir. onto yoksa panel sonraki bir sew ile baglanir; bagsiz kalirsa komsuluk_bagli kirmizi.", "en": "Add a closed panel; with onto it is applied onto a host panel's face."},
    "drop": {"args": {"panel": "string", "finish": "enum:finish"}, "tr": "PANEL KALDIR (2026-09-09): panel ve ona degen her dikis kaldirilir; o dikislerde kalan kenarlar serbest (cut) olur, finish zorunlu. Halka/kisit referanslari temizlenir. Yuze dikili (onto) parcasi olan panel, o parca kaldirilmadan kaldirilamaz.", "en": "Remove a panel and every seam touching it; the remaining edges become cut edges."},
    "merge": {"args": {"seam": "string", "panelA": "string", "panelB": "string", "panel": "string (yeni id)"}, "tr": "PANEL BIRLESTIR (2026-09-09): iki paneli aralarindaki dikis boyunca tek panele diker; dikis (o iki panel icin) kalkar. Her panelde dikis kenarlari halkada bitisik bir kosu olmali; kosunun icinde kalan pens bacaklari IC HALKA PENSE donusur (Panel.darts, balik pensi: A'nin agzi + B'nin apeksi; esi yoksa ucgen) ve reason'a adiyla yazilir. Kavsak koseler yapisal esit olmali; degilse adiyla red. Catisan kenar id'leri .1 (A) / .2 (B) olur, referanslar tasinir.", "en": "Join two panels along the seam between them into one panel; the seam is removed."}
  }
}
)stitchu"; }
