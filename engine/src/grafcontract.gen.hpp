...
// contract/graf-v1.json (35262 bayt)
inline const char* kGrafContractJSON() { return R"stitchu({
  ...
  "cozucu": {
    ...
    "pens": {
      "_tanim": "cozPens (grafop.cpp) ve supresyon kapisi (grafdogrula.cpp), A4 2026-09-09. Pens agzi = panelin HALKA CIFTINDEKI supresyon x pensPayi; halka cifti panel uyeliginden okunur (rolu 'hip' olan halkaya uye panel = etek: hip - bel; degilse bust - bel). ON/ARKA AYRI: supresyon bedenin arkaPay'i ile bolunur (arka = ust x arkaPay(ust) - alt x arkaPay(alt), on = kalan); on ve arka pens agzi bu yuzden farklidir, uydurulan on/arka sayisi yok. Yarim panel: kendi tarafinin supresyonunun yarisi, o taraftaki pens sayisina bolunur. KAPI (supresyon): tautolojik degil — cozucunun yazdigi agiz orani degil, dikilen alt halkanin (bel) cevresi ile bedenin cevresi + bollugu arasindaki EMILMEYEN fark olculur; |emilmeyen| <= toleranslar.dikisUzunlukMM.",
      "pensPayi": {
