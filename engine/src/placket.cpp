...
bool apply(DraftedPattern& pattern, double bustApexY) {
    ...
    for (size_t i = 0; i < cmds.size(); ++i) {
        ...
        if (isCloseEdge) {
            // Offset the CF-edge curve outward; it now arrives at (-standWidth,
            // neckY) instead of the neck point, and a line closes the top jog.
            c.to = outward(c.to, -standWidth);
            c.cp1 = outward(c.cp1, -standWidth);
            c.cp2 = outward(c.cp2, -standWidth);
            grown.push_back(c);
    ...
    // Fold line marking at the true CF (x = 0), from the neck edge to the bottom.
    ...
    const double facingX = standWidth; // facing folds back this far past CF (inward)
    ...
    for (double y : ys) {
        // Button: a short cross tick on the CF line.
        front->markings.push_back(PathCommand::move({-4, y}));
        front->markings.push_back(PathCommand::line({4, y}));
        // Buttonhole: a horizontal slit starting 3 mm past CF (toward the edge,
        // i.e. more negative x on this frame) and running its length outward.
        const double holeStart = -buttonholeOffset;
    ...
    pattern.guideSteps.push_back(
        "Front placket: this front carries an 18 mm grown-on button stand past "
