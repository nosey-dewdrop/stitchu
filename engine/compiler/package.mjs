...
const MM = 72 / 25.4;
const OUT = process.argv[3] || join(homedir(), 'Desktop', 'stitchu-numune-01');
...
// FAZ 5 (2026-07-22): SPEC + OUT paket parametresi (argv). package(spec,outDir)
// olarak da cagirilir. FAZ 0 elle id82 paketiyle byte-karsilastirmali.
const _ARG = process.argv[2] ? JSON.parse(process.argv[2]) : null;
const SPEC = _ARG || {
...
const out = JSON.parse(engine.draftJSON({
  ...
  gatherType: SPEC.gatherType||0, gatherZone: SPEC.gatherZone||0, backOpening: 0, backSlit: 0, ruffledStraps: 0, peplum: SPEC.peplum||0,
  tieClosureX: SPEC.tieClosure||0,
