...
var P={};
function defaults(styleKey,keep){var st=STYLE[styleKey],o={style:styleKey};for(var k in SHARED)o[k]=(keep&&k in keep)?keep[k]:SHARED[k];for(var j in st.own)o[j]=st.own[j];if(st.topLength!=null)o.topLength=st.topLength;o.title=st.label;return o;}
...
function plainSleeve(p,k){var outX=k.stX+p.sleeveWidth*S;var hemY=k.stY+p.sleeveLen*S;var wristX=outX-p.sleeveWidth*S*0.14;var g=[];
  // omuz ucundan dış kenar: düz, hafif cap yuvarlaması
  g.push(seg([k.stX,k.stY],[k.stX+(outX-k.stX)*0.55,k.stY-1],[outX,k.stY+(hemY-k.stY)*0.06],[outX,k.stY+(hemY-k.stY)*0.14]));
  // dış kenar aşağı bileğe (hafif daralma)
  g.push(seg([outX,k.stY+(hemY-k.stY)*0.14],[outX,k.stY+(hemY-k.stY)*0.6],[wristX+1,hemY-4],[wristX,hemY]));
  // bilek (kol ağzı, hafif kavis)
  g.push(seg([wristX,hemY],[wristX-(wristX-(k.uaX+8))*0.4,hemY+3],[k.uaX+12,hemY+2],[k.uaX+8,hemY-2]));
  // iç kenar (koltukaltı dikişi) bileğten underarm'a
  g.push(seg([k.uaX+8,hemY-2],[k.uaX+3,hemY-(hemY-k.uaY)*0.5],[k.uaX+1,k.uaY+10],[k.uaX,k.uaY]));
