...
Durum (K6 OPERASYONEL SERTLEŞTİRME + ARŞİV, 2026-07-19, patch 3.25): **FULL 27/54, ELEMENT 74/103, vision %94.4, korpus %6.7 — DEĞİŞMEDİ (bu ray sayaç değil operasyon ölçer; motor/golden/vision dokunulmadı, 0 çağrı).** scripts/deploy.sh doğdu: ?v bump TÜM sayfalarda otomatik + tek-sürüm mandalı, kanıt zinciri kapı (style-lint+render-lint+preview-truth+header-diff+validate-contract+ctest 45/45), motor guard (kanıtsız engine deploy'u FAIL), worker-URL drift guard; SON DEPLOY onunla yapıldı (?v 95→96, canlı curl teyit). GİZLİLİK TARAMASI: sızıntı yok, rotasyon gerekmiyor (.benchmark-token git geçmişine hiç girmemiş; KV id kimlik, secret değil; APP_TOKEN/CLAUDE_API_KEY yalnız wrangler secret; CLAUDE_API_KEY rotasyon hatırlatması Damla'da). ARŞİV: 20+ tekil proof/mock/asset dosyası docs/archive/'a; SPECS-next-vocabulary.md UNVERIFIED+PARK damgalı; vocab-canonical.json sızıntı değil → commit (%6.7 artık repodan yeniden üretilebilir). patches.html 6 em-dash temizlendi (style-lint kapısı fiilen yeşil). K0 envanter-5 satırları SIFIR. Rapor: reports/2026-07-19-stitchu-k6-operasyon.md.

Durum (K5 VISION KASKAD + EVAL TABANI, 2026-07-19, patch 3.24): **FULL 27/54, ELEMENT 74/103, vision %94.4, korpus %6.7 — DEĞİŞMEDİ (bu ray sayaç değil KREDİ EĞRİSİ ölçer; motor/golden/vision şeması dokunulmadı).** KREDİ: 1 canlı çağrı (probe, anchor 6/6 doğru), tavan 200; kalan her ölçüm önbellekli teacher etiketlerine karşı offline. STUDENT (dürüst beyan): 4 lokal mobilenet başı {garment, neckline, sleeveLength, skirtStyle} 1600 ambar etiketinden damıtıldı (AMBAR filtre istatistikleri baş başına yayınlı; GLOBAL sızıntısız md5 foto ayrımı, 4 baş paylaşır). τ KALİBRASYONU (VAL-only, hedef ≥%95 teacher-agreement): sleeveLength τ=0.94 (%95.3, kapsama %83) + skirtStyle τ=0.90 (%97.1, kapsama %34) düz eşikle; garment/neckline düz eşik TUTMADI, sınıf-koşullu τ (skirt@0.50 17/17, crew@0.95 20/21); τ'suz alan/sınıf HER ZAMAN teacher. ÇAĞRI/100 FOTO: **100 → 97.5** (val 354 fotonun 9'u atlandı, atlanan kararlarda agreement 18/18) — kazanç küçük ve DÜRÜST: fren garment başı (0.780, dress/top %95'e ulaşmıyor). ROUTER: engine/tools/cascade-router.mjs KODDA, BAYRAK KAPALI; PUBLIC yol değişmedi; bayrak + eval-gate (≥150 hand-label) İKİSİ olmadan student cevabı kullanılamaz. EVAL TABANI **KIRMIZI-DÜRÜST**: 21 → 150 hedefi el emeği ister; katman-dengeli 150 aday seçildi (filtre: seen 1600 / kept 1586 / no_photo 14), lokal etiketleme aracı (dataset/eval/label-tool.mjs, teacher cevabı gizli, kontrat enum İD'leri) + sabah paketi (dataset/eval/README.md, ~25-30 dk) hazır. DAĞILIM-KAYMASI BULGUSU: 21'lik eski müze/runway hand-label setinde kalibre τ transfer ETMİYOR (student sadece e-ticaret korpus dağılımında güvenli — kredinin yandığı yer zaten orası). Rapor: reports/2026-07-19-stitchu-k5-vision-kaskad.md.

Durum (K3 PREVIEW-TRUTH, 2026-07-19, patch 3.23): **FULL 27/54, ELEMENT 74/103, vision %94.4, korpus %6.7 — HEPSİ DEĞİŞMEDİ (bu ray sayaç değil sınır ölçer; motor C++ koduna DOKUNULMADI, wasm derlenmedi, golden by-construction aynı, ctest golden testi geçti).** contract/preview-truth.json doğdu: 4 flat stilinin her birine TEK semantik giysi kaydı; iki projeksiyon (DISPLAY = styles.json flat reçetesi, DRAFT = web/vendor canlı wasm draftJSON) aynı kayıttan türetilip diff'lenir. YAPISAL EŞİTLİK (sert): flat'in çizdiği her yapısal öğe (sleeve/collar/shirr panosu/casing/tie/backSeam) kalıpta kesim parçası bulmalı, her kesim parçası flat'te temsil edilmeli — beyansız boşluk FAIL; beyanlı boşluklar gerekçeli allowlist'te (K2 declaredButNotDrawn straps/ruffledStraps; lace×3 flat-çizer-kalıp-kesmiyor, dantel hazır şerit + listing notu; cfGather ink-sınıfı dekor). LANDMARK (yumuşak %8, büst çapası normalize): süpürme +%0.1 ve pano kesim oranı ±%0.1 (drawstring 1.800/1.8, shirred 1.998/2.0 — kontrat oran tablosu iki motorda TEK gerçek, kanıtlandı); stilizasyon sınıfı 8 landmark kontrat referanslı PİNLİ zarfta (yaka eni +%99..141, oyuntu +%68, bel −%17 review.waistNip, etek boyu +%15 flat.len beyanı, kol cut-vs-worn −%38..79). MANDAL: ctest preview_truth_check (45. test) + style-lint zincirine kanca (deploy kanıt adımı otomatik koşar) + K0 4.3 paylaşılan-packer bekçisi (assembled preview sheet.js packPieces'ten ayrılırsa FAIL). Mandal mutasyonla kanıtlı: pin daraltma FAIL, allowlist silme FAIL, draft'tan parça düşürme FAIL. KANIT: ctest 45/45, style-lint 81 sayfa + render-lint + preview-truth yeşil, flat CLI SVG 4 stil cmp bayt-aynı (export-only değişiklik). Rapor: reports/2026-07-19-stitchu-k3-preview-truth.md.

Durum (K2 KOMPOZİSYON RESMİLEŞTİRME, 2026-07-19, patch 3.22): **FULL 27/54 (PARTIAL 10), ELEMENT 74/103 (%71.8), vision %94.4, korpus %6.7, 0 unmapped — HEPSİ DEĞİŞMEDİ, 0-çağrı cache reclassify ile yeniden kanıtlandı (bu ray sayı oynatmaz, davranışı beyan eder).** contract/composition.json doğdu: 21 bileşene attachment point + z-order (garment.cpp post-pass sırası) + çakışma sınıfı; çakışma matrisi 74 host kuralı + 58 çift kuralı, 5 dürüst sınıf (allowed / excluded / validator / honest / ignored). MANDAL: ctest compose_check (248 tekil + 465 çift GERÇEKTEN çizdirilir, beyan-gözlem kayması FAIL; beyan edilmemiş sessiz no-op = FAIL) + ctest flat_render_lint (styles.json'daki her stil: self-intersect, ters sarım, sıfır alan, sampleX y-monotonluk; style-lint'e zincirli). MİKRO-LOOP 1: 20 sessiz dispatcher no-op'u (skirt×bodice bileşenleri, top×backSlit/ruffleHem, gathered×backSlit/hemShape) validateSpecCross'ta isimli redde çevrildi — web picker'ları bunları zaten engelliyordu, benchmark hükümleri birebir aynı kaldı. DÜRÜST BULGULAR (düzeltilmedi, beyan edildi): top×bardot validator-bloke, skirt×tie bel tutarsızlığı, keyhole×CF ailesi validator-bloke, 2 pinli flat self-intersection (render-lint.allow.json), flat şemasının çizmediği straps/ruffledStraps/gatherWaist bayrakları. KANIT: ctest 43/43 (K4 rebase sonrası birleşik ağaçta 44/44), golden pristine cmp byte-identical (23406), vocab-sweep 48600/0, web-fuzz 26260/3 (bilinen), iki wasm derli (worker deploy ORKESTRATÖRDE), flat CLI SVG'leri cmp bayt-aynı, render PNG gözle, style-lint 81+render-lint yeşil, header-diff 52 temiz. Rapor: reports/2026-07-19-stitchu-k2-kompozisyon.md.
