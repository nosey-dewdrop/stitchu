...
// F-İNDİR 2. tur (2026-08-26): the five contract tables arrive as a MODULE, not
// as five readFileSync calls. That was the only thing keeping this pen out of a
// browser, and a flat that cannot run in a browser cannot be downloaded from one.
// The tables are emitted by engine/tools/gen-flat-tables.mjs and any drift from
// contract/ is a red gate (ctest -R flat_tables_check), so this is a build
// artefact of the contract, not a second copy of it.
import { LAW, SICIL, VOCAB, GRAMMAR, V1V2 } from './flat-tables.gen.js';
...
async function tryReferencePen(spec) {
  ...
  try { ref = await import('../../engine/flat-engine/_engine-full.mjs'); } catch { return null; }
