...
// ⭐ BUZGULU ETEGIN DOKUM CIZGILERI — contract sevkPoz.buzgu.etekDokumu.
// Uc sayi da GIRDI/iyi-flat/adaylar/08-empire-buzgu-etek.png uzerinde piksel
// piksel olculdu (olcum penceresi ve yontem contract'ta yazili): yari basina
// UC cizgi, yari-genisligin 0.19/0.40/0.66'sinda, etek boyunun sirasiyla
// 0.56/0.63/0.41 derinliginde baslayip etek ucuna inerek.
const DOKUM_X_ORAN = [0.19, 0.40, 0.66];
const DOKUM_BASLANGIC = [0.56, 0.63, 0.41];
...
/**
 * ⭐ PENS DIKISE DEGER (T4-hakem, kusur 2). ÖLÇÜLEN HATA: indirilen her flat'te
 * ön/arka bedende 4-8 adet, hiçbir dikişe DEĞMEYEN serbest düz çizgi parçası
 * vardı ("çizime çizik atılmış gibi"). Kök sebep ölçüldü, hipotez değil:
 *
 *   Pens ağzı (`closeDart` -> fold[0]) pens KAPANMASININ bıraktığı KİNK'in tam
 *   üstünde durur. Çizilen bel dikişi ise o kink'i temizleyen eğridir
 *   (`smoothEdge`, G2-goz/K2). Yani ağzı taşıyan nokta, çizilen çizgiden
 *   sistematik olarak DIŞARIDA kalıyor. EU38 "gathered mini" elbisede ölçüldü:
 *   ön pensler 9.1 / 6.0 mm, arka pensler 5.5 / 4.5 mm boşlukta duruyordu.
 *   Etek pensleri daha da kötü: onların ağzı eteğin KENDİ ham üst kenarında,
 *   ve o kenar bir elbisede HİÇ ÇİZİLMİYOR (çizilen, bedenin bel dikişi).
 *
 * Onarım kök sebebe: pens bacağı, kendi doğrultusunda uzatılarak ÇİZİLEN
 * dikişin üstüne oturtulur. Yön pensin kendi ekseni (apeks -> ağız), hedef
 * çizilen polyline; hiçbir koordinat seçilmez, kesişim bulunur. Kesişim yoksa
 * (pens dikişi ıskalıyorsa) en yakın nokta izdüşümü alınır. İkisi de
 * bulunamazsa bacak OLDUĞU GİBİ kalır ve sebep adıyla yazılır — sessiz
 * düzeltme yok.
 */
function pensiDikiseOturt(bacak, dikis) {
  if (!Array.isArray(bacak) || bacak.length < 2 || !Array.isArray(dikis) || dikis.length < 2) return null;
  const apeks = bacak[bacak.length - 1], agiz = bacak[0];
  const v = sub(agiz, apeks), L = norm(v);
  if (!(L > EPS)) return null;
  const d = scale(v, 1 / L);
  // Kesişim: apeks + t*d, t > 0. Ağzın ötesine de bakılır (uzatma) ve berisine
  // de (kırpma), ama pensin apeksini geçmesine izin verilmez.
  let enIyi = null;
  for (let i = 1; i < dikis.length; i++) {
    const a = dikis[i - 1], b = dikis[i];
    const e = sub(b, a);
    const det = d[0] * (-e[1]) - d[1] * (-e[0]);
    if (Math.abs(det) < 1e-9) continue;
    const rhs = sub(a, apeks);
    const t = (rhs[0] * (-e[1]) - rhs[1] * (-e[0])) / det;
    const u = (d[0] * rhs[1] - d[1] * rhs[0]) / det;
    if (t <= EPS || u < -1e-6 || u > 1 + 1e-6) continue;
    if (enIyi === null || Math.abs(t - L) < Math.abs(enIyi - L)) enIyi = t;
  }
  if (enIyi !== null) return [add(apeks, scale(d, enIyi)), apeks];
  // Kesişim yok: en yakın nokta.
  let en = null, enD = Infinity;
  for (let i = 1; i < dikis.length; i++) {
    const a = dikis[i - 1], b = dikis[i], e = sub(b, a), le2 = e[0] * e[0] + e[1] * e[1];
    if (le2 < 1e-12) continue;
    const w = sub(agiz, a);
    const u = clamp((w[0] * e[0] + w[1] * e[1]) / le2, 0, 1);
    const q = add(a, scale(e, u)), dd = norm(sub(agiz, q));
    if (dd < enD) { enD = dd; en = q; }
  }
  return en ? [en, apeks] : null;
}
...
function buzguIsaretleri(pts, adet, uzunlukMM, yon = null) {
  ...
  for (let n = 0; n < adet; n++) {
    ...
    // 'asagi': the comb hangs BELOW the seam. Same law as 'disari' — the comb
    // stands on the GATHERED piece — applied to a horizontal seam: at the
    // waist the gathered piece is the skirt, and the skirt is the larger-y
    // side (the pattern's y grows toward the hem).
    else if (yon === 'asagi') { if (nrm[1] < 0) nrm = [-nrm[0], -nrm[1]]; out.push([p, add(p, scale(nrm, uzunlukMM))]); }
...
/** AYNA KACIGINI KAPAT (M6-vitrin, hakem K2 borcunun ikinci yarisi).
 *  Kalibin arka govde paneli ayna ekseninin 9.01 mm (EU38, olculdu) DISINDA
 *  baslar; bu bir kalip gerçegi (arka orta bir DIKIS, katlama degil). Arka orta
 *  cizgisi bu yuzden eksene oturtuldu (x -> 0, asagida). Ama BEL DIKISI ayni
 *  panelin ayni kosesinde bitiyordu ve eksene oturtulmadigi icin, aynalanan iki
 *  yari arasinda 2 x 9.01 = 18.02 mm'lik bir BOSLUK kaliyordu: bel dikisi arka
 *  gorunumun tam ortasinda kesiliyor, giysi belinden yarik gibi okunuyordu.
 *  Onbes satici referansinin (GIRDI/iyi-flat/adaylar) hicbirinde bel dikisi
 *  ortadan kesik degil. Cozum ayni cozum: kenarin MERKEZ ucu eksene tasinir,
 *  kaydirma yan dikise dogru sifira RAMP edilir; yani yan kose (kapilarin
 *  olctugu nokta, warpWaistPt) bayt-ayni kalir, sadece ayna kacigi kapanir.
 *  Kaydirma miktari uydurulmaz: kenarin kendi merkez ucunun x'i kadardir. */
function eksenKapa(pts) {
  if (!pts || pts.length < 2) return pts;
  const n = pts.length - 1;
  // Merkez uc = eksene YAKIN olan uc (bedende son, etekte ilk gelir).
  const merkezSon = Math.abs(pts[n][0]) <= Math.abs(pts[0][0]);
  const dx = merkezSon ? pts[n][0] : pts[0][0];
  if (Math.abs(dx) <= 0.5) return eksenDik(pts);  // zaten eksende, sadece tegeti dikle
  return eksenDik(pts.map(([x, y], k) => [x - dx * (merkezSon ? k / n : 1 - k / n), y]));
}

/** AYNA CIZGISINE DIK BITIS (M6-vitrin, hakem Kirmizi 2).
 *  OLCULEN KUSUR: yukaridaki kaydirma kenarin merkez ucunun X'ini eksene
 *  oturtuyor ama TEGETINI dikmiyor. Kaydirma her adimda ayni dx/n kadar
 *  oldugu icin merkeze yaklastikca yatay adim kisaliyor, egim ARTIYOR:
 *  flat-kumas-dokuma-eu38.svg on bel dikisinde eksendeki teget yataya 9.55
 *  derece, aynalaninca on ortada 19.1 derecelik SIVRI bir V kosesi (yan bele
 *  gore 16.78 mm asagi centik). Hakem: "motor bunu 'V bel' diye ilan etmiyorsa
 *  artefakttir". Onbes satici referansinin (GIRDI/iyi-flat/adaylar/13) on beli
 *  duz/yumusak; sivri V yok.
 *
 *  YASA, kusurun SINIFI icin (tek bir cizime gore degil): ayna ekseninde biten
 *  her ic cizgi eksene DIK bitmek zorundadir — aksi halde aynalanan cift bir
 *  kose yapar ve o kose giyside olmayan bir V'dir.
 *
 *  INSA, SERBEST PARAMETRE YOK: iki ucundan gecen ve merkez ucta tegeti yatay
 *  olan cember YAYI tektir (merkezi eksende, (0, yC + r); r kenarin kendi iki
 *  ucundan cozulur). Uydurulan sayi yok, secilen "harmanlama boyu" yok. Iki uc
 *  BAYT-AYNI kalir (yan kose = warpWaistPt, kapilarin olctugu nokta).
 *  MANDAL: yay kenarin kendi noktalarindan 12 mm'den fazla sapiyorsa kenar
 *  yay-benzeri degildir; o zaman cizim DEGISTIRILMEZ (kapi (h2c) kirmizi yanar
 *  ve kusur adiyla gorunur) — kalibin desteklemedigi bir sekil cizilmez.
 *  12 mm esigi bu dosyanin smoothEdge mandaliyla ayni esiktir. */
function eksenDik(pts) {
  if (!pts || pts.length < 3) return pts;
  const ilk = pts[0], son = pts[pts.length - 1];
  const merkezSon = Math.abs(son[0]) <= Math.abs(ilk[0]);
  const C = merkezSon ? son : ilk;          // ayna eksenindeki uc
  const Y = merkezSon ? ilk : son;          // yan uc (bayt-ayni kalir)
  const dx = Y[0] - C[0], dy = Y[1] - C[1];
  if (Math.abs(dx) < 1) return pts;         // yatay uzanimi yok
  if (Math.abs(dy) < 0.05) return pts;      // zaten duz: duz cizgi eksene dik
  const r = (dx * dx + dy * dy) / (2 * dy); // isaretli yaricap, merkez (C[0], C[1]+r)
  const cx = C[0], cy = C[1] + r, R = Math.abs(r);
  const a0 = Math.atan2(C[1] - cy, C[0] - cx);
  let d = Math.atan2(Y[1] - cy, Y[0] - cx) - a0;
  while (d > Math.PI) d -= 2 * Math.PI;
  while (d < -Math.PI) d += 2 * Math.PI;
  const N = Math.max(16, pts.length - 1);
  const yay = [];
  for (let k = 0; k <= N; k++) {
    const a = a0 + d * (k / N);
    yay.push([cx + R * Math.cos(a), cy + R * Math.sin(a)]);
  }
  yay[0] = C.slice(); yay[yay.length - 1] = Y.slice();
  // Mandal: kenarin kendi noktalari yaydan ne kadar uzakta?
  const kaynak = merkezSon ? pts.slice().reverse() : pts;   // C -> Y yonunde
  const ync = cumFrac(yay), pnc = cumFrac(kaynak);
  let sapma = 0;
  for (let k = 0; k <= N; k++) {
    const p = atFrac(kaynak, pnc, k / N).p;
    const q = atFrac(yay, ync, k / N).p;
    sapma = Math.max(sapma, Math.hypot(p[0] - q[0], p[1] - q[1]));
  }
  if (sapma > 12) return pts;
  return merkezSon ? yay.reverse() : yay;
}
...
function buildView(P, which) {
  ...
  if (bod) {
    ...
    if (skirtP) {
      ...
      interior.push(['bel-dikisi', eksenKapa(smoothEdge(sewn.edgePts)), true]);
      // ⭐ M5-buzgu — BEL DIKISININ PAYDASI. Darts closed, this polyline IS the
      // length the skirt has to be sewn onto. It is stashed so the skirt branch
      // can hold its own waist edge against it; nothing is assumed about the
      // skirt's style, the two drafted lengths are simply compared.
      out.notes.bodiceWaistMM = poliUzunluk(sewn.edgePts);
    } else {
    ...
    // ⭐ ARKA ORTA DIKIS TEK YOLDUR (M6-vitrin, hakem K2 borcu).
    // Kalibin arka orta kenari ayna cizgisinden 8.47 mm (EU38) DISARIDA durur —
    // bu bir KALIP gercegi (dikis payi tarafi), giysinin gorunusu degil. O kenar
    // oldugu yerde cizilip aynalaninca sayfaya IKI dikey cizgi basiyordu ve
    // arka, ortasindan yarik bir giysi gibi okunuyordu. Onbes satici
    // referansinin hepsinde (GIRDI/iyi-flat/adaylar, ozellikle 13-yuksek-bel-
    // a-line.png) arka orta dikis TEK bir cizgidir ve tam ortadan gecer.
    // Cozum kenari silmek degil, onu kendi ayna eksenine oturtmak: x -> 0.
    // Boylece sag yol ile ayna yolu USTUSTE duser = tek yol. Kenarin y'si,
    // dolayisiyla boyu ve baslangic/bitis noktalari, kalibin kendi olcusudur;
    // sadece ayna kacigi kapanir. Siluet bu kenardan cizilmiyor (siluet ayri
    // kapali yol), o yuzden giysinin dis hattina dokunmaz.
    ...
    if (d.centre.length && !/placket|pleat/.test(kapanisAdi) &&
      ...
      const eksende = d.centre.map((s) => ({ ...s, p: s.p.map(([, y]) => [0, y]) }));
      interior.push(['orta-dikis', eksende, false]);
    }
  ...
  if (skirtP) {
    ...
    // ⭐ M5-buzgu — BÜZGÜLÜ ETEK, ÇİZİMDE BÜZGÜSÜZ ÇIKIYORDU.
    //
    // MEASURED 2026-09-04 (hakem turu 1, kusur 4). `skirtStyle: 'gathered'`
    // ile çizdirilen elbisenin başlığı "Gathered ... dress" oluyor, parça
    // listesi doğru, ama İNDİRİLEN teknik flat'te belde tek bir büzgü tiki
    // yok — düz A-line görünüyordu. Kök sebep bu dosyanın kendi ölçüsünde:
    // `buzguOku` YALNIZCA KOLU ölçüyor (kapak = sleeve_cap vs armhole,
    // etek = sleeve_hem vs cuff). Etek-bel dikişi hiç ölçülmüyordu, çünkü
    // motorun etek parçaları HİÇ edgeRoles taşımıyor (ölçüldü: "Skirt Front ||"
    // — rol listesi boş), yani rol tabanlı ölçüm oraya hiç uzanamıyordu.
    //
    // Ölçü yine kalıptan, yine iki kenarın karşılaştırılması: eteğin dikilen
    // (pens kapanmış) BEL kenarı, bedenin dikilen bel kenarına bölünür. Aynı
    // eşik (contract draft.gatherRatios.sleeveCapGathered = 1.235) çünkü aynı
    // ayrım: bunun altı yedirme, üstü büzgü. Ölçülen: EU38 elbisede
    // gathered 354.4 / 247 civarı ≈ 1.43 (büzgü), aLine ≈ 0.87 (büzgü değil).
    if (out.notes.bodiceWaistMM > 0) {
      const etekBelMM = poliUzunluk(sewn.edgePts);
      const oran = etekBelMM / out.notes.bodiceWaistMM;
      if (etekBelMM > 0 && oran >= BUZGU_ESIK) {
        out.notes.buzguBel = { capMM: etekBelMM, hedefMM: out.notes.bodiceWaistMM,
                               oran, fazlaMM: etekBelMM - out.notes.bodiceWaistMM };
      }
    }
    ...
    for (const leg of sewn.legs) {
      ...
      // 4. eleman: bu bacak KEMER tarafından kasten kısaltıldıysa aşağıdaki
      // "pensi dikişe oturt" adımı onu geri uzatmamalı — kemer zaten ağzını
      // örtüyor. Kısaltılmamış bacak dikişe oturtulur.
      interior.push(['pens', L, true, wbDepth > 0]);
    }
    ...
    if (wbDepth > 0) {
      ...
      // Ayni ayna yasasi (eksenDik): kemer cizgisi de eksene DIK biter, yoksa
      // aynalanan cift belin ortasinda bir V kosesi yapar.
      if (sl.length > 1) interior.push(['bel-dikisi', eksenKapa(sl), true]);
      interior.push(['bel-dikisi', eksenKapa(w), true]);
    }
  }
  ...
  // ⭐ PENS AĞZI ÇİZİLEN DİKİŞE OTURUR (T4-hakem kusur 2). Gerekçe ve ölçüm
  // `pensiDikiseOturt`'un başında. Hedef, çizimde GERÇEKTEN basılan bel dikişi
  // (en uzun 'bel-dikisi' polyline'ı); bel dikişi olmayan bir üstte hedef,
  // silüetin kendi etek ucu kenarıdır. Boşluk ÖLÇÜLÜP ilan edilir, sessizce
  // kapatılmaz.
  {
    let hedef = null;
    for (const e of interior) {
      if (e[0] !== 'bel-dikisi' || !e[2] || !Array.isArray(e[1]) || e[1].length < 2) continue;
      if (!hedef || e[1].length > hedef.length) hedef = e[1];
    }
    if (!hedef && out.notes.hemPts && out.notes.hemPts.length > 1 && !skirtP) hedef = out.notes.hemPts;
    if (hedef) {
      let enBuyukMM = 0, oturan = 0, kacan = 0;
      for (const e of interior) {
        if (e[0] !== 'pens' || !e[2] || e[3]) continue;
        const yeni = pensiDikiseOturt(e[1], hedef);
        if (!yeni) { kacan++; continue; }
        const fark = norm(sub(yeni[0], e[1][0]));
        if (fark > enBuyukMM) enBuyukMM = fark;
        e[1] = yeni;
        oturan++;
      }
      if (oturan) out.notes.pensOturmaMM = enBuyukMM;
      if (kacan) out.sebep.push(`${which}: ${kacan} pens bacagi cizilen dikise oturtulamadi`);
    }
  }
  ...
  // =========================================================================
  {
    ...
    // ⭐ PATA ETEK UCUNA KADAR INER + DUGME DAIREDIR (hakem, 2026-09-03).
    // ESKI HAL, iki kusur: (1) pata yalniz BEDEN parcasindan okunuyordu, bel
    // dikisinde kesiliyordu — olculdu, K4: kapama y = 133.14..424.76, giysi
    // 1205 boyunda. Dugmeli bir shirttail'de belde biten pata yanlis okunur.
    // Kok sebep KALIPTAYDI (etek on paneli patayi hic tasimiyordu) ve orada
    // duzeltildi: PlacketBlock artik etek on panelini de aciyor. Cizim de artik
    // patayi TASIYAN her parcadan okur. (2) dugme, iki kisa YATAY cizgi olarak
    // basiliyordu ve patanin solunda tarak gibi bir tirtik cikiyordu; teknik
    // flat konvansiyonunda dugme DAIREDIR, ilik ise kisa bir yariktir.
    // Dairenin yaricapi uydurulmadi: kalibin kendi dugme tikinin yarisi.
    if (F && bod) {
      const patalar = [];
      if (/placket/.test(kapanis)) patalar.push([bod.piece, esle(bod.piece, fY0, yBel), sxUst]);
      if (skirtP && /placket/.test(skirtP.piece.closure || '')) {
        patalar.push([skirtP.piece, esle(skirtP.piece, yBel, fY1), sxAlt]);
      }
      for (const [pc, mY, sxP] of patalar) {
        const m = pc.markings || [];
        const kb = pc.commands.filter((c) => c.type !== 'close');
        const boy = Math.max(...kb.map((c) => c.y)) - Math.min(...kb.map((c) => c.y));
        // Dugme mu ilik mi? ESIK SECILMEDI, VERIDEN OKUNDU: pata isaretlerinin
        // kisa YATAY olanlari tam iki uzunlukta gelir — dugme tiki ve ilik
        // yarigi (kalibin kendi sayilari, placket.cpp). Kisa olan aile dugme,
        // uzun olan ilik. (Ilk deneme patanin katlama cizgisinin x'ini arayip
        // "tiki ortalayan = dugme" diyordu; TELA/pata kenari da uzun bir dikey
        // oldugu icin foldX'i o kapiyordu ve K4'te 13 dugmenin 13'u ilik diye
        // cizildi — olculdu: kapama-dugme 0, kapama-ilik 26.)
        const kisaYatay = [];
        for (let i = 0; i + 1 < m.length; i++) {
          if (m[i].type !== 'move' || m[i + 1].type !== 'line') continue;
          const dx = Math.abs(m[i].x - m[i + 1].x), dy = Math.abs(m[i].y - m[i + 1].y);
          if (dy < 0.5 && dx > 1 && dx < 40) kisaYatay.push(dx);
        }
        const dugmeBoy = kisaYatay.length ? Math.min(...kisaYatay) : null;
        for (let i = 0; i + 1 < m.length; i++) {
          if (m[i].type !== 'move' || m[i + 1].type !== 'line') continue;
          const a = m[i], b = m[i + 1];
          const dx = Math.abs(a.x - b.x), dy = Math.abs(a.y - b.y);
          if (dx < 0.5 && dy > boy * 0.5) {
            oz('kapama', [[a.x * sxP, mY(a.y)], [b.x * sxP, mY(b.y)]], W_SEAM, 'kapama',
               sxP, ` data-kapama-x-mm="${a.x.toFixed(2)}"`);
          } else if (dy < 0.5 && dx > 1 && dx < 40) {
            const orta = (a.x + b.x) / 2;
            const dugmeMi = dugmeBoy !== null && Math.abs(dx - dugmeBoy) < 0.5;
            if (dugmeMi) {
              const r = (dx / 2) * sxP, cxq = orta * sxP, cyq = mY(a.y);
              const d = `M ${(cxq - r).toFixed(4)},${flipY(cyq).toFixed(4)} ` +
                        `a ${r.toFixed(4)},${r.toFixed(4)} 0 1 0 ${(2 * r).toFixed(4)},0 ` +
                        `a ${r.toFixed(4)},${r.toFixed(4)} 0 1 0 ${(-2 * r).toFixed(4)},0`;
              push('kapama', d, W_SEAM,
                   ` data-view="front" data-ozellik="kapama-dugme" data-olcek="${sxP.toFixed(6)}"` +
                   ` data-dugme-cap-mm="${(dx).toFixed(2)}"`,
                   [[cxq - r, cyq - r], [cxq + r, cyq + r]]);
            } else {
              oz('kapama', [[a.x * sxP, mY(a.y)], [b.x * sxP, mY(b.y)]], W_TOPSTITCH,
                 'kapama-ilik', sxP);
            }
          }
        }
    ...
    // --- KRUVAZE (surplice / wrap on). HAKEM KIRMIZI 1 (M6-vitrin): landing
    // "surplice front" diye basiyordu, cizimde ne capraz kenar ne bindirme
    // vardi — simetrik V yaka + duz dikey on orta. Kalip DOGRUYDU: wrapfront.cpp
    // on paneli on ortayi ASAN tek parcaya cevirir (EU38'de x ekseni -110.0 ..
    // +244.2 mm, "cut 2 (mirror wrap — right laps over left)"), yani kalipta
    // gercek bir kruvaze var; CIZIM onu okumuyordu. Bu dosyanin yasasi geregi
    // (giysinin ADINI veren ozellik cizimde aranir) artik okunuyor.
    //
    // GIYSIDE GORUNEN TEK CIZGI: iki on birbirinin uzerine biner, yani ALTTA
    // kalan onun capraz kenari ustteki panelin altinda kaybolur. Gorunen, USTTE
    // kalan onun (kadin konvansiyonu: SAG on sola biner) capraz kenaridir ve o
    // kenar kalipta zaten TEK duz cizgidir: on orta yaka noktasindan (0, yaka)
    // wrap kosesine (-wrapPastCF, bel). Cizime tasima olcegi uydurulmaz, bu
    // bloktaki her sey gibi kalibin kendi mm'si x sxUst.
    if (F && bod && /wrap|surplice/i.test(kapanis)) {
      const kb = bod.piece.commands.filter((c) => c.type !== 'close');
      const asma = Math.abs(Math.min(0, Math.min(...kb.map((c) => c.x))));  // wrapPastCF, mm
      if (asma > 1) {
        const xw = asma * sxUst;
        // Bel dikisinin o x'teki kendi y'si (bel duz degil, kavisli).
        let yw = yBel;
        if (belSeam && belSeam.length > 1) {
          let en = Infinity;
          for (const p of belSeam) {
            const dq = Math.abs(Math.abs(p[0]) - xw);
            if (dq < en) { en = dq; yw = p[1]; }
          }
        }
        oz('kruvaze', [[0, fY0], [xw, yw]], W_SEAM, 'kruvaze-kenar', sxUst,
           ` data-kruvaze-asma-mm="${asma.toFixed(2)}"`);
      }
    }
    ...
    if (P.katmanlar && P.katmanlar.length && kapaliPts.length) {
      ...
      // ⭐ ZIGGURAT KUSURU KAPANDI (hakem, 2026-09-03). ESKI HAL: her katmanin
      // cizilen yarim genisligi kendi ACIK (buzgulenmemis) cevresinin ceyregiydi
      // — K3 EU38'de tier 1 -> +/-299.63 mm, tier 3 -> +/-340.48 mm, etek ucu ise
      // bambaska. Sonuc, giysinin siluetine baglanmayan, her biri farkli enli UC
      // AYRI DIKDORTGEN: basamakli bir pasta. Kok sebep, kalipta degil CIZIMDE
      // idi: bir katmanin ACIK uzunlugu KALIBIN isidir, resmin degil.
      //
      // YENI KANUN (konvansiyon): buzgulu katman, TUTUNDUGU kenar kadar genis
      // baslar (basamak imkansiz), yan kenari giysinin KENDI olculen etek
      // egimini surdurur (duz etekte duz, A-line'da acilmaya devam eder), ve
      // fazlalik BUZGU olarak gosterilir + data-katman-buzgu-orani ile ILAN
      // edilir. Hicbir kesir secilmedi: baslangic eni cizimden, egim cizimden,
      // oran kalibin kendi cevresinden.
      const yariGen = (yy, tol = 10) => {
        let en = null;
        for (const q of kapaliPts) {
          if (Math.abs(q[1] - yy) <= tol && (en === null || Math.abs(q[0]) > en)) en = Math.abs(q[0]);
        }
        return en;
      };
      const hemW = yariGen(fY1);
      // Etegin KENDI egimi: hem ile hemin (etek boyunun) dortte biri yukarisi
      // arasindaki yarim-genislik degisimi / mm. Olculemezse 0 (duz iner).
      const olcuBoy = Math.max(40, (fY1 - yBel) * 0.25);
      ...
      if (wUst !== null && wUst > 0) for (const p of sira) {
        ...
        // ⭐ KATMAN ARTIK BEYAN ETTIGI DOLGUNLUK KADAR GENISLIYOR.
        //
        // OLCULDU 2026-09-04 (hakem): K3'un dort katman konturunun DORDU de
        // -239.70 ... +239.70 mm idi; data-katman-buzgu-orani="1.2500" yaziyor
        // ama geometri 1 mm bile genislemiyordu. Beyan ile geometri iki ayri
        // kaynaktan geliyordu ve aralarinda kapi yoktu.
        //
        // Bir onceki tur bunu bilerek yapmisti ve gerekcesi DOGRUYDU: kalibin
        // ILAN ETTIGI sayiya oturunca viewBox 15 metreye cikiyordu. Ama kok
        // sebep cizimde degil KALIPTA idi — ruffle.cpp yayinlanmis tek-kenar
        // buzgu oranini (2.0-3.0 bandi) HER DIKISTE yeniden uyguluyordu
        // (2.5^3 = 15.6). O kok sebep bu turda kapatildi (bkz. ruffle.cpp
        // M7 notu: katman basina oran = fullness^(1/tiers), son katman tam
        // olarak hem x fullness). Kalip artik giyilebilir sayilar veriyor, o
        // yuzden cizim artik kalibin kendi sayisina oturuyor:
        //   alt yarim genislik = (bitmis alt cevre / 4) x bolge olcegi.
        // Ceyrek, ust kenarin kendi tanimiyla ayni (tutunanCevre asagida ayni
        // formulun tersidir), yani beyan ile geometri TEK sayidan turuyor.
        // EU38 K3: 239.7 -> 325 -> 441 -> 599 mm yarim genislik.
        //
        // Etegin kendi egimi (`egim`) yalnizca cevresini ILAN ETMEYEN bir
        // katman icin yedek kalir — o zaman cizilecek baska sayi yoktur.
        const ilanEdilen = acikCevreMM(p);
        const wAlt = Math.max(1, ilanEdilen !== null && sxAlt > 0
          ? (ilanEdilen / 4) * sxAlt
          : wUst + egim * derin);
        ...
        // Ruffle tier 3'un acik cevresi 1361.9 mm — 373.1 mm eksik. Kok sebep
        ...
        // Buzgu orani = katmanin ACIK cevresi / tutundugu kenarin cevresi.
        // Tutundugu kenarin kalip cevresi, cizilen yarim genisligin bolge
        // olcegiyle geri cevrilip dorde katlanmasidir (ceyrek -> tam cevre).
        const tutunanCevre = sxAlt > 0 ? (wUst / sxAlt) * 4 : 0;
        const oran = tutunanCevre > 1 ? uzun / tutunanCevre : 0;
        const ek = ` data-katman="${p.name.replace(/"/g, '')}"` +
                   ` data-katman-derinlik-mm="${derin.toFixed(2)}"` +
                   ` data-katman-cevre-mm="${uzun.toFixed(2)}"` +
                   ` data-katman-tutunan-cevre-mm="${tutunanCevre.toFixed(2)}"` +
                   ` data-katman-buzgu-orani="${oran.toFixed(4)}"${eksik}`;
        oncekiCevre = uzun;
        oz('katman', [[-wUst, y], [wUst, y]], W_SEAM, 'katman-dikis', sxAlt, ek);
        oz('katman', [[-wUst, y], [-wAlt, y + derin], [wAlt, y + derin], [wUst, y]], W_OUTLINE,
           'katman-kontur', sxAlt, ek);
        const adet = Math.max(1, Math.round(1 / BUZGU_TARAK_ARALIK_ORAN) - 1);
        for (const [q0, q1] of buzguIsaretleri([[-wUst, y], [wUst, y]], adet, Math.min(derin * 0.4, 14))) {
  ...
  let belGeom = null;   // the DRAWN waist seam (post-warp, post-CF clip)
  for (const [rol, geomHam, isPoly] of interior) {
    ...
    if (rol === 'bel-dikisi' && isPoly && !belGeom) belGeom = geom;
  }

  // ⭐ M5-buzgu — BEL BÜZGÜ TARAĞI. Aynı notasyon, aynı yoğunluk, ikinci dikiş.
  // Kol kapağı büzgüsünün tarağı (contract flat-convention-v1 sevkPoz.buzgu.tarak,
  // referans 09'da piksel piksel sayıldı) bu dosyada zaten çiziliyordu; büzgülü
  // etek onunla AYNI cinsten bir dikiş (iki yanı farklı uzunlukta) olduğu halde
  // hiçbir işaret almıyordu. Yeni bir kural yok: mevcut kanunun, ölçülmüş
  // yoğunluğuyla, ikinci bir büzgülü dikişe uygulanması. Tarak BÜZÜLEN parçanın
  // tarafında durur; belde büzülen parça etektir, o yüzden tikler dikişin
  // ALTINA iner ('asagi').
  if (out.notes.buzguBel && belGeom && belGeom.length > 1) {
    const bb = out.notes.buzguBel;
    const adet = Math.max(1, Math.round(1 / BUZGU_TARAK_ARALIK_ORAN) - 1);
    const tik = Math.max(6, ((skirtP && skirtP.piece.seamAllowance) || 15) * 0.7);
    const uzun = poliUzunluk(belGeom);
    const bAttr = (yan) => ` data-view="${F ? 'front' : 'back'}" data-yan="${yan}"` +
      ` data-buzgu="bel" data-buzgu-oran="${bb.oran.toFixed(4)}"` +
      ` data-buzgu-dikis-mm="${uzun.toFixed(2)}"` +
      ` data-buzgu-fazla-mm="${bb.fazlaMM.toFixed(2)}"`;
    for (const [a, b] of buzguIsaretleri(belGeom, adet, tik, 'asagi')) {
      push('buzgu', polyD([a, b], flipY), W_TOPSTITCH, bAttr('sag'), [a, b]);
      const m = [[-a[0], a[1]], [-b[0], b[1]]];
      push('buzgu', polyD(m, flipY), W_TOPSTITCH, bAttr('sol'), m);
    }

    // ⭐ ETEGIN DOKUMU (T4-hakem, kusur 1). Hakem, buzgulu etegi "dumduz
    // kenarli bir UCGEN KONI, govdesinde tek bir buzgu/drape cizgisi yok" diye
    // adlandirdi ve haklıydi: buzgu YALNIZ bel dikisinde bir tarak olarak
    // vardi, kumasin altta ne yaptigi hic cizilmiyordu. Buzgulu bir dikisin
    // altindaki kumas duz durmaz. Cizgilerin sayisi, yatay konumu ve nereden
    // basladigi SECILMEDI: ucu de referans 08'de olculdu (contract
    // sevkPoz.buzgu.etekDokumu._kaynak). Cizgi, silüetin O YUKSEKLIKTEKI
    // kendi yari-genisliginin bir kesridir, yani etek genisledikce cizgiler de
    // onunla acilir — sabit bir egri degil, silüetin turevi.
    const etekPts = closed.flatMap((sg) => sg.p);
    const yBelD = Math.min(...belGeom.map((p) => p[1]));
    const yEtekUcu = etekPts.length ? Math.max(...etekPts.map((p) => p[1])) : yBelD;
    if (yEtekUcu > yBelD + 1) {
      const yariGenislik = (y) => {
        let en = 0;
        for (const q of etekPts) if (Math.abs(q[1] - y) <= 6) en = Math.max(en, Math.abs(q[0]));
        return en;
      };
      const dAttr = (yan, k) => ` data-view="${F ? 'front' : 'back'}" data-yan="${yan}"` +
        ` data-dokum="etek" data-dokum-x-oran="${DOKUM_X_ORAN[k].toFixed(2)}"` +
        ` data-dokum-baslangic="${DOKUM_BASLANGIC[k].toFixed(2)}"` +
        ` data-buzgu-oran="${bb.oran.toFixed(4)}"`;
      for (let k = 0; k < DOKUM_X_ORAN.length; k++) {
        const y0 = yBelD + DOKUM_BASLANGIC[k] * (yEtekUcu - yBelD);
        const pts = [];
        const N = 16;
        for (let n = 0; n <= N; n++) {
          const y = y0 + (n / N) * (yEtekUcu - y0);
          const w = yariGenislik(y);
          if (!(w > 0)) continue;
          pts.push([DOKUM_X_ORAN[k] * w, y]);
        }
        if (pts.length < 2) continue;
        push('dokum', polyD(pts, flipY), W_TOPSTITCH, dAttr('sag', k), pts);
        const m = pts.map((q) => [-q[0], q[1]]);
        push('dokum', polyD(m, flipY), W_TOPSTITCH, dAttr('sol', k), m);
      }
    }
  }
  ...
  if (out.notes.sleeve) {
    ...
    const yay = (A, B, fazlaMM, yonTercih = null, krestSonda = false) => {
      ...
      let dik = [d[1], -d[0]];
      // Varsayilan yon +x (kapak buzgusunun kabarmasi). BALON kolda kabarma
      // KATLAMA CIZGISINDEN uzaga gider (kolun kendi genisligi o yonde olculur),
      // o yuzden yon disaridan verilebilir; verilince +x kurali degil, verilen
      // vektorle ayni tarafta olan dik secilir.
      if (yonTercih) { if (dik[0] * yonTercih[0] + dik[1] * yonTercih[1] < 0) dik = [-dik[0], -dik[1]]; }
      else if (dik[0] < 0) dik = [-dik[0], -dik[1]];
      ...
      // Ayni yasa, iki ucta: "kabarma BUZGUNUN oldugu yerde durur". Kapak
      // buzgusunde buzgu A ucunda (omuz), balon kolun ETEK buzgusunde B ucunda
      // (manset). O yuzden profil, buzgunun hangi ucta oldugunu soyleyen
      // `krestSonda` ile aynalanir; egrinin kendisi ayni egridir.
      const bump = krestSonda ? (t) => Math.sin(Math.PI * Math.sqrt(1 - t))
                              : (t) => Math.sin(Math.PI * Math.sqrt(t));
      const cizgi = (amp) => {
        const pts = [];
        for (let i = 0; i <= N; i++) {
          const t = i / N;
    ...
    // ⭐ BALON KOL: MANSET, KOLUN BUZGULU ETEK GENISLIGINDE CIZILIYORDU
    //    (T4-hakem, kusur 1 — "kagit ucak kanadi").
    //
    // OLCULEN HATA (EU38, 'a dress with a gathered skirt and balloon sleeves,
    // mini'): kalip `Balloon Sleeve` + `Sleeve Cuff` basiyor ve buzguOku etek
    // buzgu oranini 1.8005 olarak OKUYOR — kolun etek kenari, mansetin bitmis
    // boyunun 1.8 kati. Buna ragmen cizim manset cizgisini kolun KENDI genis
    // etek yarisi olan 169.45 mm'ye basiyordu; mansetin gercek yarisi
    // 169.45 / 1.8005 = 94.1 mm. Manset kolla ayni genislikte cizilince balon
    // kolun tek tanimlayici ozelligi — bilekte KISILMESI — silinir ve geriye
    // duz bir kama kalir. Oran zaten OLCULUYORDU, cizime hic uygulanmiyordu.
    //
    // ONARIM, HER SAYISI KALIPTAN. Balon kol + manset dort bilinen uzunlukla
    // cizilir ve dordu de kalibin kendi milimetresidir:
    //   Lh    kolun cizilmis etek YARISI (katlama -> koltukalti dikisi)
    //   Lc    mansetin BITMIS yarisi = Lh / buzgu orani
    //   derin mansetin kendi kisa kenari (stripDepth(P.cuff))
    //   under kolun kendi koltukalti kenari
    // Kontur: koltukalti kenari, manset dikisi hizasindaki TAM kumas
    // genisligine (Lh) kadar iner; orada kumas mansete buzulur (Lh -> Lc);
    // manset kendi derinliginde duz iner; etek Lc genisligindedir. Yani
    // balonun kabarmasi = buzgu fazlasinin ta kendisi (Lh - Lc), ve nerede
    // kisildigi da secilmedi: mansetin kendi derinliginde.
    let innCiz = g.inn, underCiz = g.under;
    let balonKabarmaMM = 0, mansetYarimMM = 0;
    const mansetDerin = P.cuff ? stripDepth(P.cuff) : 0;
    if (bz.etek && bz.etek.oran > 1 && g.Lh > EPS && mansetDerin > EPS) {
      const perp = unit(sub(g.inn, g.out));
      const Lc = g.Lh / bz.etek.oran;                       // mansetin bitmis yarisi
      const cIn = add(sub(g.out, scale(g.d, mansetDerin)), scale(perp, Lc));  // manset dikisinin ic kosesi
      innCiz = add(g.out, scale(perp, Lc));                 // etek (mansetin alti)
      mansetYarimMM = Lc;
      balonKabarmaMM = g.Lh - Lc;
      // Koltukalti konturu: U'dan mansetin ic kosesine giden kiris, kolun
      // KENDI etek yarisi kadar (Lh - Lc = olculen buzgu fazlasi) katlama
      // cizgisinden UZAGA kabartilir. Kabarmanin tepesi buzgunun oldugu ucta
      // (manset) durur — kapak buzgusunde tepe omuzda duruyor, ayni yasa.
      // Genlik SECILMEDI: giysinin o hizadaki gercek kumas yarisi Lh, mansetin
      // bitmis yarisi Lc, aradaki fark tam olarak kabaran kumastir.
      const N = 28;
      const kabar = (t) => Math.sin(Math.PI * Math.sqrt(1 - t));
      const govde = [];
      for (let i = 0; i <= N; i++) {
        const t = i / N;
        govde.push(add(lerp(g.U, cIn, t), scale(perp, balonKabarmaMM * kabar(t))));
      }
      underCiz = govde.concat([innCiz]);
    }
    const ustKenar = bz.kapak ? yay(g.S, g.out, bz.kapak.fazlaMM / 2) : [g.S, g.out];
    const shape = ustKenar.concat(underCiz.slice().reverse());
    ...
    if (depth > 0) {
      ...
      const c = [sub(g.out, scale(g.d, depth)), sub(innCiz, scale(g.d, depth))];
      push(rolM, polyD(c, flipY), wM, ` data-view="${F ? 'front' : 'back'}" data-yan="sag"${ekM}` +
           (balonKabarmaMM > 0 ? ` data-manset-yarim-mm="${mansetYarimMM.toFixed(2)}"` +
                                 ` data-kol-etek-yarim-mm="${g.Lh.toFixed(2)}"` +
                                 ` data-balon-kabarma-mm="${balonKabarmaMM.toFixed(2)}"` : ''), c, dM);
