...
// ⭐ DECLARED, NOT COPIED (H3-B). This hex was written here bare, and a bare hex
// is a second truth: the flat law's ink and the printed pattern's ink were the
// same colour by coincidence-of-typing, with nothing holding them together. The
// trailing declaration is machine-read by engine/tests/flat_mirror_check.mjs,
// which resolves the dotted path against the file on disk on every run.
const NAVY = HEX('#1f3a5f');   // contract/flat-convention-v1.json ink.color
...
// F9 (2026-09-02): the FULL-SIZE check square. The 3 cm square above stays (it
// fits the cover margin and reads with any ruler), but 3 cm is small enough
// that a 2-3% printer scale error hides inside the pencil line (0.6-0.9 mm).
// At 100 mm the same error is 2-3 mm — unmissable. Indie sellers ship a
// 10 cm test square for exactly this reason, so the A4 pack now carries one on
// its own page, drawn by the same Ctx mm pipeline as every pattern line.
export function calibration100(ctx, x, y) {
  ctx.stroke(0.5, INK); ctx.dash(null);
  ctx.rect(x, y, 100, 100, 'S');
  // mid ticks so a half-reading (5 cm) is checkable too
  ctx.line(x + 50, y, x + 50, y + 3);
  ctx.line(x + 50, y + 97, x + 50, y + 100);
  ctx.line(x, y + 50, x + 3, y + 50);
  ctx.line(x + 97, y + 50, x + 100, y + 50);
  ctx.text(x + 50, y + 108, 4.2, INK, '10 cm = 100 mm, measure me before cutting', 'middle');
  ctx.text(x + 50, y + 114, 3.4, GREY, 'printer scale 100%, headers/footers off. If this square is not 100 mm, do not cut.', 'middle');
}
...
export function makePdfCore({ engine, sheet, body }) {
  ...
  function a4Pdf(s, p, layout, sheets, used) {
    ...
    // F9: dedicated calibration page — the 100 mm square plus the seam-
    // allowance declaration, right after the cover, before any pattern sheet.
    // On its own page because 100+ mm of clear paper cannot be promised on a
    // cover whose cut list grows with the garment.
    {
      const c = new Ctx(A4.h);
      const M = 18;
      c.text(M, 28, 16, NAVY, 'Calibration / Kalibrasyon', null);
      const saCm = (p.pieces[0].seamAllowance / 10).toFixed(1);
      const saMM = Math.round(p.pieces[0].seamAllowance);
      c.text(M, 40, 9.5, INK, `seam allowance ${saMM} mm (${saCm} cm) included in the cut line: cut the OUTER line, sew the inner fine line.`, null);
      c.text(M, 46, 9.5, INK, `dikis payi ${saMM} mm DAHIL: DIS cizgiden kes, icteki ince cizgiden dik.`, null);
      calibration100(c, (A4.w - 100) / 2, 70);
      pdf.page(A4.w, A4.h, c.s);
    }
  ...
  function buildAll(s, outDir, writeFileSync, join) {
    ...
    return { slug: s.slug, pieces: p.pieces.length, a4pages: sheets.length + 2,
