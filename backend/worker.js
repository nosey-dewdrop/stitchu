...
// F1: /api/analyze artık METİN de kabul ediyor (fotoğraf + prompt hedef
// cümlenin yarısı). Prompt kurulumu saf modülde (analyze-core.js) yaşar ki
// düz node'dan test edilebilsin; foto istem baytları DEĞİŞMEDİ (parity testi
// engine/tests/prompt_spec_check.mjs'te).
import { buildAnalyzeRequest } from './analyze-core.js';
...
// Garment analysis. The app sends a base64 JPEG, a text description, or both;
// we prompt Claude (prompt assembly + validation in analyze-core.js) and
...
async function handleAnalyze(request, env, overrideModel = null, preParsed = null) {
  ...
  // Foto, metin ya da ikisi birden (F1). Kurulum ve doğrulama saf modülde
  // (analyze-core.js); her red adıyla döner, sessiz default yok. Foto+metin
  // birlikte geldiğinde istem, yazının fotoğrafı EZDİĞİNİ modele de söyler
  // (öncelik kuralı istemcide de aynı).
  const built = buildAnalyzeRequest(body, overrideModel || MODEL);
  if (built.error) {
    return jsonResponse({ error: built.error, detail: built.detail }, built.status);
  }
  const anthropicBody = built.body;
