...
import { draftRecipe, loadEngine } from './engine.js?v=139';
...
function downloadFactoryPack() {
  ...
  a.href = `factory/${pack.file}?v=139`;
...
async function loadRecipe(entry) {
  const res = await fetch(`recipes/${entry.file}?v=139`);
...
async function init() {
  ...
  try {
    const res = await fetch('recipes/index.json?v=139');
  ...
  try {
    const fres = await fetch('factory/index.json?v=139');
