/* clip-v0.js — Track B v0: LOCAL zero-shot garment attribute classification.
   ...
   run:  node vision/clip-v0.js   -> vision/eval/siglip-predictions.json */
const fs = require('fs');
...
(async () => {
  ...
  const clip = await pipeline('zero-shot-image-classification', 'Xenova/siglip-base-patch16-224');
  ...
  fs.writeFileSync(path.join(__dirname, 'eval/siglip-predictions.json'), JSON.stringify(out, null, 1));
  console.log('\nwrote vision/eval/siglip-predictions.json');
})();
