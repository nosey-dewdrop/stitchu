/* clip-v0.js — Track B v0: LOCAL zero-shot garment attribute classification.
   ...
   run:  node vision/clip-v0.js   -> vision/eval/fashionclip-predictions.json */
const fs = require('fs');
...
(async () => {
  ...
  const clip = await pipeline('zero-shot-image-classification', 'Xenova/fashion-clip');
  ...
  fs.writeFileSync(path.join(__dirname, 'eval/fashionclip-predictions.json'), JSON.stringify(out, null, 1));
  console.log('\nwrote vision/eval/fashionclip-predictions.json');
})();
