...
struct BodiceDraft {
    ...
    /// Waist width of one drafted half-piece (sewing line, mm) — used by
    /// dress/top blocks to match skirt/hem widths.
...
struct BodiceBlock {
    ...
    static func draft(measurements m: BodyMeasurementsSnapshot) -> BodiceDraft {
        let neck = m.neckMM
        ...
        let backNeckW = neck * backNeckWidthFactor
        ...
        let back = piece(
            ...
            centerCutInstruction: false
        )

        // ---- FRONT (cut 1 on fold, all suppression in the waist dart) ----
        let frontNeckW = neck * frontNeckWidthFactor
        let frontCutout = frontNeckW + 15 // classic front scoop: width + ~1.5cm
        ...
        let front = piece(
            ...
            dartWidth: frontDart,
            dartLength: frontLength - armholeY - 40, // apex stops short of bust point
            centerTakeIn: 0,
            centerCutInstruction: true
        )
    ...
    private static func piece(
        ...
        let commands: [PathCommand] = [
            .move(centerNeck),
            // neck curve up to the shoulder-neck point
            .curve(to: neckPoint,
                   cp1: CGPoint(x: neckW * 0.55, y: neckCutout),
                   cp2: CGPoint(x: neckW * 0.9, y: neckCutout * 0.35)),
            .line(shoulderTip),
            // armhole: shoulder tip curving into the side seam
            .curve(to: armholeBottom,
