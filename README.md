...
**the outside reference: real-product benchmark.** a hand-labeled set of real garment photos, measured end-to-end. **counting method corrected 2026-07-18:** the old counter asked "did a pattern come out" and never drafted the spec, so a spec whose sleeve silently fell to none still counted FULL. the counter now DRAFTS every FULL candidate through the real engine and requires (1) a promised sleeve to produce a sleeve piece, (2) no classified field to fall silently to a default, (3) every element counted as drawable to leave evidence in the drafted pieces. published side by side, not silently replaced: **old method 34/54 on the current cache (previously reported 37/54; the 3-photo gap is not reproducible from the repo cache and is itself a finding), honest method 23/54 full + 11 partial.** the number dropped because the old number was a count, not a measurement; 23 is where the chain really stands and the 11 partials are the exact repair list. the misses are published too, in the patch notes. vision accuracy on critical fields: 94% against hand labels. this is the honest measure of "did a real dress turn into a real pattern"; it is the number that moves when the engine actually learns to draw something new.
