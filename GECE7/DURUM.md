...
| **F7** | ⏳ ajan bekliyor | açılmadı | — | ⏳ **KART YAZILDI: `GECE7/F7.md`.** ⚠ **Hanesi H1 DEĞİL** — §3.6 H1 veriyordu ama H1 **tavanda** (5/5 · 10/10) ve tavandaki bir sayıyı hane yapmak *"hiçbir şey yapma"*yı ödüllendirir; hakem §3.7 yetkisiyle haneyi **`H8-ifade`nin PAYI: 3/5 → EN AZ 4/5** yaptı. **Gerekçe uydurma değil, ÖLÇÜLMÜŞ bir kuyruk**: `expressability_check` paydayı **adıyla** basıyor ve `freesewing-aaron`'u bloke eden iki operatör **`op.extend` · `op.attach`** — yani §3.12'nin F7'ye yazdığı işin ta kendisi (*uzat · fiyonk ekle*). `bugra-locket-top` **bu kartın şartı DEĞİL** (dört operatör ister, F5'in kuyruğu). **Payda 5 MÜHÜRLÜ** (K17) — paydayı büyüterek hane kazanılmaz. İkinci iş **borç 86** (örme × kollu DXF boş) ve **bir KAPIYLA** kapanır; üçüncü iş **borç 88** (oyuk ve büzgü bir tabana bağlı değil) |
