export const meta = {
  ...
  phases: [
    { title: 'A1 Gecit', detail: 'kapi.sh, sessiz regresyon seti, state.json, fotograf hatti saglik (emsal olcum 0-K 1 ile kalkti)' },
    ...
    { title: 'A4 Cizim', detail: 'flat = mankene giyilmis izdusum, satici flat karikaturu (0-K 3a); kalip sayfasi standardi; 34-44' },
...
// ============================================================ 15 SATIR KURAL (9.1) ============================================================
const ORTAK = `
stitchu 0509 kosusunda TAZE bir ajansin; isin bitince olursun, her sey dosyaya ve commit'e. Repo: ${REPO}. Ilk komut: cd ${REPO} && (GIT_TERMINAL_PROMPT=0 timeout 60 git pull --rebase origin main || echo 'PULL DUSTU (yerel HEAD ile devam, is kaybolmaz)') && git status --short && git log --oneline -8.
...
12. Git: main, branch yok; her anlamli adimda commit + push; mesaj kucuk harf Ingilizce, co-author yok. git clean/stash/reset --hard yasak. Silme ayri commit, rg kaniti mesajda. UZUN KOMUT SESSIZ KALMAZ (2026-09-08, olculdu: A3 iscisi 304 adim gercek is yapti, sonra harness'in 180 SANIYELIK 'ilerleme yok' tavanina takilip kesildi ve ardindan 5 deneme daha ayni yerde ANINDA oldu; is commit'lenmisti, kaybolmadi ama adim bitmedi). 180 sn'den uzun surebilecek her komut (tam ctest, 0509-kapi.sh tam mod, cmake tam build, Chrome PNG uretimi, 5 fotograflik toplu uretim) ARKA PLANDA kosulur ve beklenirken cikti uretilir:
   - Bash tool'unu run_in_background:true ile cagir, sonra BashOutput ile ara ara oku; ya da
   - komutu 'KOSU/0509-<is>.log' dosyasina yonlendirip & ile arka plana at, wait ETME, arada 'tail -5' ile son satirlari bas.
   ASLA: onplanda 3 dakikadan uzun tek komut. Toplu uretimi (5 fotograf) TEK komutta degil, fotograf basina AYRI komutta kos — her biri kendi ciktisini basar, sessizlik olusmaz. Bir komutun ne kadar surecegini bilmiyorsan arka plana al.
PULL DA ASLA INTERAKTIF OLMAZ: her git pull TAM OLARAK su bicimdedir: GIT_TERMINAL_PROMPT=0 timeout 60 git pull --rebase origin main (duserse echo ile isaretle); duserse YEREL HEAD ile devam edilir, adim DUSMEZ, deneme YANMAZ. Kimlik dogrulama, remote degistirme, credential helper denemesi YASAK. PUSH ASLA INTERAKTIF OLMAZ (2026-09-08, Damla karari; A3 iscisi 'git push -q origin main' uzerinde 12 dakika asili kaldi, hicbir cikti vermeden, adim hic baslamadan). Her push TAM OLARAK su bicimde kosulur:
   GIT_TERMINAL_PROMPT=0 timeout 60 git push origin main
   Push DUSERSE (exit != 0, timeout dahil): commit ZATEN atilmistir ve KALIR, is kaybolmaz. Koşu DURMAZ, adim DUSMEZ, deneme YANMAZ. Yalnizca KOSU/0509-state.json'a "pushBekliyor": true yazilir (varsa listeye commit sha eklenir) ve yapilan[] icine "push bekliyor: <sha> (<hata satiri>)" konur. Damla sonra elle pushlar. Push hatasini onarmaya CALISMA: kimlik dogrulama, remote degistirme, --force, ssh anahtari, credential helper YASAK. Tag push'lari da ayni sekilde: GIT_TERMINAL_PROMPT=0 timeout 60 git push -f origin <tag> — duserse tag YERELDE kalir, yeter, devam et.
...
const EL_ORTAK = `
stitchu 0509 kosusunun MEKANIK ELISIN. Yargi vermezsin, kod yazmazsin, onarmazsin; yalniz verilen komutlari kosar, dosyalari yazar ve sonucu SAYIYLA raporlarsin. Repo: ${REPO}; once cd ${REPO} && (GIT_TERMINAL_PROMPT=0 timeout 60 git pull --rebase origin main || echo 'PULL DUSTU (yerel HEAD ile devam, is kaybolmaz)'). Alt-ajan yok. Komut ciktisinin son 30 satirini oku, yorum ekleme. Bir komut yoksa (ornek ${KAPI} henuz yazilmadi) "yok" de, uydurma. Bitince commit + push (yalniz KOSU/0509-* ve ${DOC} §5 degisiklikleri). PULL DA ASLA INTERAKTIF OLMAZ: her git pull TAM OLARAK su bicimdedir: GIT_TERMINAL_PROMPT=0 timeout 60 git pull --rebase origin main (duserse echo ile isaretle); duserse YEREL HEAD ile devam edilir, adim DUSMEZ, deneme YANMAZ. Kimlik dogrulama, remote degistirme, credential helper denemesi YASAK. PUSH ASLA INTERAKTIF OLMAZ (2026-09-08, Damla karari; A3 iscisi 'git push -q origin main' uzerinde 12 dakika asili kaldi, hicbir cikti vermeden, adim hic baslamadan). Her push TAM OLARAK su bicimde kosulur:
   GIT_TERMINAL_PROMPT=0 timeout 60 git push origin main
   Push DUSERSE (exit != 0, timeout dahil): commit ZATEN atilmistir ve KALIR, is kaybolmaz. Koşu DURMAZ, adim DUSMEZ, deneme YANMAZ. Yalnizca KOSU/0509-state.json'a "pushBekliyor": true yazilir (varsa listeye commit sha eklenir) ve yapilan[] icine "push bekliyor: <sha> (<hata satiri>)" konur. Damla sonra elle pushlar. Push hatasini onarmaya CALISMA: kimlik dogrulama, remote degistirme, --force, ssh anahtari, credential helper YASAK. Tag push'lari da ayni sekilde: GIT_TERMINAL_PROMPT=0 timeout 60 git push -f origin <tag> — duserse tag YERELDE kalir, yeter, devam et. Hizli ol.
`
...
ADIM A1 — GECIT ALTYAPISI. Kosucu (0509-kosu.js) zaten uyarlandi. Iki isci (ALT ADIM satirina bak): A1a = madde 1, 4, 9, 10 (kapi.sh iskeleti + kilit + metrik + state + devir notu); A1b = madde 2, 3, 5, 6, 7, 8 (emsal olcum, regresyon seti, wasm sanity, olcek, onbellek, saglik). URUN DEGISMEZ; motor koduna dokunma. Her alt adim kendi butcesiyle.
 ...
 2. [KALDIRILDI 2026-09-09, Damla 0-K 1: emsal flat seti silindi; kapi emsal olcmez] (eski: 0509-emsal-olcum.mjs, K5 emsal cizimleri, KOSU/ciktilar/flat-olcum.json + contract/flat-convention-v1.json. Esikler contract'tan okunur, koda gomulmez. KOSU/flat-olcum.py'yi oku ve kullan; yeniden yazma.
...
A2b'YE KARAR AJANI NOTLARI (2026-09-06, A2 ilk gecis kararlari — baglayici):
 - URUN ONCE: A2b'nin URUN teslimi degismedi. ILK SAATTE kotu de olsa KOSU/ciktilar/graf-ilk/flat.png + kalip-36.png commit'te olacak. Asagidaki uc arac onarimi (Q1/Q2/Q3) o teslimi geciktirirse ARAC ONARIMI KESILIR, urun once cikar, kalan arac onarimi A2c'ye kayar.
 - olcek_check KAPALI — YENIDEN ACMA YOK. Gecit A2'de kapandi (YESIL, 990.00 mm, aralik [395,1335]); 0509-kapi.sh olcek_gecit() bugun grafciz'i FIILEN kosuyor, HENUZ-YOK yalnizca contract/body-v1.json'da olcekAraligi hic yoksa doner (commit b8f3f4d9). Bu maddeyi devredilen soru diye tasima.
 - GENEL DERS (devir hijyeni): devredilen bir soruyu tasimadan ONCE ilgili dosya SATIRINI yeniden oku. Arada onaran bir commit atilmis olabilir; commit mesaji da dosya hali de degismis olabilir. Beyan ile disk hali ayrisir, OLCUM kazanir (ayni ders kilit tuzaginda da gecerli).
 - Q1 (esik-alti metrik) ve Q3 (H7 izin listesi) uygulandi; Q2 kosucu el('basla') icinde. Bunlarin regresyonu bu adimda kollanir.
...
Hedef (0-K 3a, 2026-09-09): GIRDI/hedef-fotograflar-3 satici flat'leri — bizim flat ilanin flat'i gibi, mankene giyilmis izdusum; emsal flat seti SILINDI, referans verilmez. Flat croquis36'da, kalip gercek36'da, ayni graf; 34-44 graded bedende YENIDEN DEGERLEME. Kalip cizimi standardi (parca etiketi, grain, notch, kat, kesim/dikis cizgisi, tipografi) pattern-sheet-v1.json'dan; eksik bir sey varsa kaynagiyla ekle (ASTM D6673, ticari indie kalip PDF'leri; patterns_real/ Bugra sayfalari yalniz OLCULUR).
Her turda bir png: KOSU/ciktilar/giris-3/a4-kontak-tur<k>.png = ilan (fotograf + satici flat'i) | bizim flat | kalip | sayilar (node KOSU/0509-a4-kontak.mjs giris-3 <k>). Kalip sayfasi: KOSU/ciktilar/_yerel/cizim/kalip-vs-bugra.png (Bugra'nin bir sayfasiyla ayni cerceve; telifli, commit yok) + commit'li KOSU/ciktilar/cizim/kalip-36.png.
Kapi hakemin mm olcumu; kosu Damla'yi beklemez. En fazla 6 tur (her tur bir commit + bir png); 6 turda satici flat'ine yaklasmiyorsa DUR, acikSorular'a "kok neden croquis mi (contract/body-v1.json croquis36) konvansiyon contract'i mi" diye olcumle yaz (8.12), karar ajani yazar.
...
KABUL: kor goz hakemi iki tur ust uste EVET (0-K 5), flat_ayni_insan_check tum flat'lerde (graf-ilk + giris/1-5 + regresyon seti) tolerans icinde, flat_convention_check yesil, seri 34-44 dogrulayici 0 kirmizi.
...
// KARAR Q1 (2026-09-07, A2 karar ajani): IZIN LISTESININ TEK KAYNAGI KOSU/0509-state.json <altAdim>IzinListesi.
// faz.izin / faz.izinAlt artik BRIEF metnidir, YETKI KAYNAGI DEGILDIR: --kilit ve --kilit-diff yalniz bu fonksiyonun
// dondurdugu listeyle cagrilir. Ilan yoksa liste BOS doner => kilit hic acilmaz, her sey salt-okunur (sessiz genis liste yok).
// Cozumleme H7'nin 0509-kapi.sh:818-841 icindekiyle ayni: en ozelden genele (<altAdim>IzinListesi, <adim>IzinListesi),
// bulanik esleme yok, "yol (aciklama)" formatinda yalniz yol kismi glob.
function stateIzinListesi(altEtiket, adimNo) {
  let st = {}
  try { st = JSON.parse(require('fs').readFileSync(STATE, 'utf8')) || {} } catch (e) { return { liste: [], izin: '', kaynak: 'state okunamadi' } }
  const adaylar = []
  if (altEtiket) adaylar.push(String(altEtiket).split(/\s+/)[0])
  if (adimNo) adaylar.push(String(adimNo))
  for (const ad of adaylar) {
    const v = st[ad + 'IzinListesi']
    if (Array.isArray(v)) {
      const liste = v.filter(g => typeof g === 'string').map(g => g.split(' (')[0].trim()).filter(Boolean)
      return { liste, izin: liste.join(' '), kaynak: `${STATE}:${ad}IzinListesi` }
    }
  }
  return { liste: [], izin: '', kaynak: `ILAN YOK (${adaylar.join(',') || 'adimsiz'})` }
}
...
async function el(gorev, adim, ek) {
  ...
  // 0509 fix (butce): alt adimi olan adimlarda tag ve butce ALT ADIM basina; A1'de A1a+A1b tek tavana carpti (26/12).
  const _altEtiket = (ek && ek.altAdim) ? String(ek.altAdim).split(/\s+/)[0] : ''
  const _tagAd = _altEtiket ? `adim-${_altEtiket}` : `adim-${adim ? adim.no : 'kosu'}`
  // KARAR Q1: --kilit ve --kilit-diff yalniz state.json ilanindan beslenir; ek.izin (brief) girdi DEGILDIR.
  const _izinK = stateIzinListesi(_altEtiket, adim ? adim.no : null)
  const _izin = _izinK.izin
  const _ilanYok = _izinK.liste.length === 0
  ...
  const gorevler = {
    basla: `ADIM BASLANGICI ${adim.ad}${_altEtiket ? ' / ' + _altEtiket : ''} (11.5):
 1. ZORUNLU ILK KOMUT (atlanamaz, baska hicbir sey bundan once yapilmaz): git tag -f ${_tagAd}-once HEAD && GIT_TERMINAL_PROMPT=0 timeout 60 git push -f origin ${_tagAd}-once || echo 'TAG PUSH DUSTU (yerel tag yeterli, devam)'. Tag adi TAM OLARAK "${_tagAd}-once" — kisaltma, adim numarasina indirgeme, "zaten var" diye atlama YOK; bu tag butcenin baslangic noktasidir, yanlissa butce yanlis sayilir ve adim haksiz yere duser. Attiktan sonra "git tag -l ${_tagAd}-once" ciktisini yapilan[] icine yaz. (yeniden acilis/tekrar ise tag ILERI tasinir: butce bu acilistan sayilir); GIT_TERMINAL_PROMPT=0 timeout 60 git push -f origin --tags || echo 'TAGS PUSH DUSTU (devam)'. REFERANS KILIDI (KARAR Q2, 2026-09-06 A2 karar ajani — kilit COMMIT'TE TASINMAZ, chmod git nesnesi degildir; bu yuzden kilit her alt adim basinda FIILEN yeniden kurulur ve durumu commit mesajindan DEGIL H7 ciktisindan okunur):
   (a) FIILEN KOS: bash ${KAPI} --kilit ${JSON.stringify(_izin)} (izin listesinin TEK KAYNAGI ${_izinK.kaynak}; brief'teki faz.izin/izinAlt YETKI DEGILDIR — KARAR Q1, 2026-09-07)${_ilanYok ? ' — ILAN YOK: izin listesi BOS, kilit acilmaz, her sey salt-okunur; adim BASLAMAZ: gecitYesil=false + kirmizilar\'a "ILAN YOK: state.json\'da <altAdim>IzinListesi yok, KARAR Q1(b)" yaz ve dur.' : ''} (yoksa: chmod -R a-w contract engine/tests engine/golden-reference.csv engine/src/grafdogrula.* engine/src/solver_utils.* 2>/dev/null; izin listesindekilere chmod u+w). Ciktinin "kilit: N dosya salt-okunur, izin listesinden M dosya yazilabilir" satirini AYNEN yapilan[] icine yaz (parafraz degil, satirin kendisi).
   (b) HEMEN ARDINDAN OKU: bash ${KAPI} --kendi-check 2>&1 | grep H7 — bu satiri da yapilan[] icine yaz. H7 KIRMIZI (FAIL) ise ADIM BASLAMAZ: isciyi calistirma, kilidi yeniden kur ((a)'yi tekrarla) ve H7 hala kirmizi ise gecitYesil=false + kirmizilar'a "H7 kilit-kurulu: <FAIL satiri>" yaz.
   (c) "lock set" gibi bir commit MESAJI tek basina kilit kaniti SAYILMAZ; kanit yalniz (a) ve (b) ciktilaridir.
  Faz yolu ${STATE}.yol dizisine eklenir (11.7 makro-ABA).
 ...
 4. Commit + push. gecitYesil = (2)'de hicbir sey kirmizi degilse.`,
    ...
    geriAl: `KILIT IHLALI GERI ALINIYOR (v2 referans kilidi): ${adim.ad} iscisi izin disi dosyalara dokundu: ${JSON.stringify(ek && ek.dosyalar || [])}. Her biri icin git checkout ${_tagAd}-once -- <dosya>; commit "revert reference-lock violation in ${adim.no}: <dosyalar>"; push. ${STATE} banned += "izin disi dokunus: <dosyalar>". Baska hicbir seye dokunma.`,
    ilan: `ILAN GECISI (KARAR Q3, 2026-09-07): ${STATE} icindeki izin ilanlari kosucu tarafindan ZATEN yazildi (dosyayi tekrar duzenleme, degistirme). Yapilan: ${JSON.stringify(ek && ek.ilanYapilan || [])}.
 1. git add ${STATE} && git commit -m ${JSON.stringify(ek && ek.ilanNot || 'ilan gecisi')} && GIT_TERMINAL_PROMPT=0 timeout 60 git push origin main. Commit mesaji TAM OLARAK bu metin.
 2. HEMEN ARDINDAN yalniz OKU (kilit KURMA; kilidi bir sonraki adim basi ${STATE}'ten okudugu YENI listeyle kuracak): bash ${KAPI} --kendi-check 2>&1 | grep H7 — H7 satirini AYNEN yapilan[] icine yaz. H7 KIRMIZI ise gecitYesil=false + kirmizilar'a 'H7 ilan gecisi sonrasi: <FAIL satiri>'.
 3. Baska hicbir seye dokunma.`,
...
# ISCININ OZETI\n${isci.ozet}\n# KARARLAR\n${kararNotu}\n` + alanBlok,
    ...
    const olcum = soruTuru ? null : await el('olc', faz, { izin, tabanKurar: !!faz.tabanKurar, altAdim })   // 0509 (8.25): butce/kilit/ivme olcumu isci VE uygula bittikten sonra (kosucu ajani kesemez; isci kendini keser, 7. kural)
    ...
    if (olcum && olcum.kilitIhlali && olcum.kilitIhlali.length) {   // v2 referans kilidi: isci kesilir, diff geri alinir, deneme duser
      await el('geriAl', faz, { dosyalar: olcum.kilitIhlali, altAdim })
...
// KARAR Q3 (2026-09-07, A2 karar ajani): bir kerelik izin kalemleri, sahibi belli ve ani belli olacak sekilde,
// YENI ALT ADIM el('basla') --kilit'ten ONCE state.json'dan DUSURULUR ve yeni alt adimin ilan alani acilir.
// Isci kendi iznini kendi ilan edemez (Q3'u bozar). Girdi tablosu: <alt adim> -> {dus:[<liste>.<kalem onEki>], ac:{<liste>:[glob]}}
const ILAN_GECIS = {
  'A2c': {
    dus: [{ liste: 'A2bIzinListesi', onEk: 'engine/tests/0509-kapi.sh' }],
    ac: { 'A2cIzinListesi': ['engine/CMakeLists.txt'] },
    not: "A2bIzinListesi: 0509-kapi.sh dusuruldu (bir kerelik izin bitti, KARAR Q3)",
  },
}
async function ilanGecisi(altAdim) {
  const anahtar = String(altAdim || '').split(/\s+/)[0]
  const g = ILAN_GECIS[anahtar]
  if (!g || KURU) return
  const yapildi = []
  try {
    const fs = require('fs')
    ...
    let degisti = false
    for (const d of (g.dus || [])) {
      const v = st[d.liste]
      if (!Array.isArray(v)) continue
      const kalan = v.filter(x => !(typeof x === 'string' && x.split(' (')[0].trim() === d.onEk))
      if (kalan.length !== v.length) { st[d.liste] = kalan; degisti = true; yapildi.push(`${d.liste} -= ${d.onEk}`) }
    }
    for (const [ad, liste] of Object.entries(g.ac || {})) {
      if (!Array.isArray(st[ad])) { st[ad] = liste; degisti = true; yapildi.push(`${ad} = ${liste.join(' ')}`) }
    }
    if (degisti) fs.writeFileSync(STATE, JSON.stringify(st, null, 2) + '\n')
  } catch (e) { gunluk.push(`ilanGecisi ${anahtar} okunamadi: ${e.message}`); return }
  if (!yapildi.length) return
  gunluk.push(`ilanGecisi ${anahtar}: ${yapildi.join('; ')}`)
  await el('ilan', { ad: `ilan gecisi ${anahtar}`, no: anahtar }, { ilanNot: g.not, ilanYapilan: yapildi })
}

async function adimKos(faz, ekNot) {
  const bas = await el('basla', faz)   // KARAR Q1: izin state.json'dan
  ...
  if (ALT[faz.no]) {   // 9.7: buyuk adim, alt adim basina isci; hakem her alt adimi olcer
    ...
    for (const alt of ALT[faz.no]) {
      // 0509 fix (butce): her alt adim kendi tag'ini alir, butce alt adim basina olculur (A1'de A1a+A1b tek tavana carpti)
      await ilanGecisi(alt)   // KARAR Q3 (2026-09-07): bir kerelik izinler yeni alt adim baslarken DUSER, sahibi kosucudur
      const bAlt = await el('basla', faz, { altAdim: alt })   // KARAR Q1: izin state.json'dan okunur, buradan verilmez
      if (!bAlt) return { gecti: false, isci: null, hukum: { neden: `ALTYAPI: el (basla) iki kez oldu (${alt})`, kusurlar: [], rewardHacking: [] }, altyapi: true }
...
for (let i = 0; i < SIRA.length; i++) {
  ...
  if (r.yenidenAc) {   // 0509 (11.7)
    ...
    phase(hedef.ad)
    await el('basla', hedef)   // KARAR Q1: izin state.json'dan
